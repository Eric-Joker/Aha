...
from utils.misc import caller_module
...
loaded_i10n: dict[str, dict[str, dict[str, str]]] = defaultdict(dict)  # dict[module, dict[lang, dict[key, value]]]
...
def get_translation(key: str, module: str = None, lang_code: str = None):
    ...
    for lang in _get_fallback_chain(lang_code):
        if (translations := loaded_i10n[module].get(lang)) and key in translations:
...
def _(key: str):
    return LocalizedString(key, caller_module())
...
def _get_fallback_chain(lang_code: str | None):
    ...
    if DEFAULT_LANGUAGE is None:
        if current_process().name == "MainProcess":
            ...
            DEFAULT_LANGUAGE = status["def_lang"]
...
async def _process_locales_directory(module: str | None, locales_path: Path):
    ...
    async for entry in locales_path.iterdir():
        if (
            not await entry.is_file()
            or entry.suffix.lower() not in YAML_EXT
        ):
            continue
