...
from utils.misc import MODULE_PATTERN, caller_module, is_subsequence
    ...
    def _rotate_if_needed(self, force=False):
        ...
        if force or self.current_size >= self.max_bytes:
            ...
            self.current_file = open(
                self.log_dir / self._get_filename(self.start_time),
                "a",
                encoding="utf-8",
            )
            self.current_size = 0
...
def setup_logging(handler: HandlerConfig = None):
    global log_config, _log_queue, _log_handler, _log_instance, _ASYNC_MODE
    if not handler:
        from core.config import cfg

        # 创建统一文件日志线/进程
        if _ASYNC_MODE := cfg.api_service_mode == "async":
...
def getLogger(name: str = None):
    if not (module := caller_module(pattern=MODULE_PATTERN)):
