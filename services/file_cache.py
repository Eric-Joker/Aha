...
class CacheFileSession:
    ...
    def __init__(self, dir: Path, name: str, ext: str):
        ...
        self.fileext = ext
        if name:
            self.path = self.dir / (name + ext if ext else name)
    ...
    async def register(
        ...
        if content is None:
            if not self.filename:
                self.filename = token_hex(16) + self.fileext if self.fileext else token_hex(16)
        ...
        if self.filename:
            ...
            if isinstance(content, (bytes, str)):
                self.filename = xxh3_128_hexdigest(content) + self.fileext if self.fileext else xxh3_128_hexdigest(content)
                is_tmp = False
            else:
...
@asynccontextmanager
async def cache_file_sessionmaker(file_name=None, file_ext=None, _level=2):
    """获取缓存文件工作会话

    Args:
        file_name: 未提供时会采用内容哈希或随机16位 hex。
        file_ext: 若有值，会在生成的路径结尾添加。（不会自动补齐点）
    """
