...
class GroupAPI:
    ...
    @staticmethod
    async def send_group_unescape_text(group_id: str | int, text: str, *, bot: int = None) -> str:
        """发送不转义的群聊文本消息。
    ...
    @staticmethod
    async def send_group_forward_msg(group_id: str | int, forward: Forward, *, bot: int = None) -> str:
        """发送群聊合并转发消息。
    ...
    @staticmethod
    async def send_group_forward_msg_by_id(group_id: str | int, messages: list[str | int], *, bot: int = None) -> str:
        """
        Returns:
            str: message_id
        """
    ...
    @staticmethod
    async def get_file(file_id: str, file: str, *, bot: int = None):
        """获取文件信息"""
