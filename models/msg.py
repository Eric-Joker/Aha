...
class MessageChain[T: MsgSeg](list[T]):
    ...
    @overload
    def __setitem__(self, key: SupportsIndex, value: T | Strable) -> None: ...
    @overload
    def __setitem__(self, key: slice, value: Iterable[T | Strable]) -> None: ...
    ...
    def __iadd__(self, iterable: Iterable[T | Strable]):
...
class Downloadable(MsgSeg):
    ...
    async def download(self, dir_=None, name=None, fix_ext=True, record_format=AudioFormat.MP3, bot_id=None):
        ...
        async with await cfm.get_session(name) as session:
            # 在标准文件缓存的远程文件
    ...
    async def stream(self, size=8192, record_format=AudioFormat.MP3, bot_id=None):
        ...
        async with await cfm.get_session(self.name) as session:
            # 在标准文件缓存的远程文件
...
class Image(Downloadable):
    ...
    async def from_pil(
        ...
        async with await cfm.get_session(name, 3) as session:
...
class Forward(MsgSeg):
    ...
    async def append(
        ...
        Args:
            ...
            bot_id: 为传入 `user_id` 或 `nickname` 时调用 `get_login_info` API 的 bot 实例。默认从实例来源 bot 或当前上下文中获取。
        """
    ...
    async def from_content(
        ...
        Args:
            nickname: 若外部已拥有或欲调用多次该方法可以传入 `user_id` 和 `nickname` 参数以减少 API 调用次数。
            bot_id: 为传入 `user_id` 或 `nickname` 时调用 `get_login_info` API 的 bot 实例。默认从当前上下文中获取。
        """
