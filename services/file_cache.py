...
class CacheFileManager(metaclass=SingletonMeta):
    ...
    def __init__(self):
        self.cache_dir = CFGS["dir"]

    async def start_service(self):
        await scheduler.add_schedule(self.cleanup, CronTrigger.from_crontab(CFGS["cleanup_cron"]))
        self.cache_dir = await self.cache_dir.resolve()

    async def get_session(self, file_name=None, _level=2):
        """获取缓存文件工作会话
        ...
        return CacheFileSession(cache_dir, file_name)
