...
值存在类型限制，必须为 `Option`、`int`、`float`、`Sequence`、`Mapping`、dataclass、[attrs](https://github.com/python-attrs/attrs)、[Pydantic Model](https://docs.pydantic.dev/latest/concepts/models/) 或 `None`。
...
从字符串中提取 [PUA](https://zh.wikipedia.org/wiki/%E7%A7%81%E4%BA%BA%E4%BD%BF%E7%94%A8%E5%8C%BA) 码点，返回 set[int]。

> 可选安装 [numpy](https://pypi.org/project/numpy) 以提升性能。

#### `gen_pua_char`

返回一个 [PUA](https://zh.wikipedia.org/wiki/%E7%A7%81%E4%BA%BA%E4%BD%BF%E7%94%A8%E5%8C%BA) 字符并将其码点注册进 `ContextVar utils.string.current_PUAs`。同一个上下文中不会返回相同的字符。

#### `InlineStr`

通过类方法 `from_iterable` 将可迭代对象的**所有元素拼接成一个字符串**，并将其中**非字符串且非 [`Text`](../../数据结构/消息序列与消息段.md#text) 实例**的对象分别通过 [`gen_pua_char()`](#gen_pua_char) **替换为一个 [PUA](https://zh.wikipedia.org/wiki/%E7%A7%81%E4%BA%BA%E4%BD%BF%E7%94%A8%E5%8C%BA)** 字符。

实例方法 `to_list` 还原上述过程并返回列表。**仅在同一上下文中才可还原！**

生成实例时会通过 [`extract_pua_ord()`](#extract_pua_ord) 将原字符串中的所有 [PUA](https://zh.wikipedia.org/wiki/%E7%A7%81%E4%BA%BA%E4%BD%BF%E7%94%A8%E5%8C%BA) 字符注册进 `ContextVar utils.string.current_PUAs`。若为通过 `from_iterable` 生成的实例，`current_PUAs` 存在原字符串中已有的 [PUA](https://zh.wikipedia.org/wiki/%E7%A7%81%E4%BA%BA%E4%BD%BF%E7%94%A8%E5%8C%BA) 时，会抛出 `models.exc.PUAConflictError`；因此**应尽早执行 [`event.get_msg_inline()`](../../数据结构/事件对象.md#message)** 等类似逻辑。
