...
>
> 黑魔法特性包括 `Apply` 运算符、[属性穿透](#属性穿透)、[pre_hook](./订阅与发布事件.md#预处理匹配的消息内容) 。
...
| `PM.limit` | bool | 触发者的限速状态，为 True 代表状态正常未限制。若总匹配条件较为宽泛应显式声明 `Plimit == False` | -999 |
...
例如 `Pmsg_chain[0]` 会生成表达式实例并在评估时提取为第一个消息段。
