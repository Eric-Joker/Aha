...
from utils.misc import caller_module, get_arg_names, get_item_by_index, get_true_func
...
class ExprPool[Key: Hashable | Expr, Attach: Any | tuple | Callable | None](Container[tuple[Key, Callable, Attach]]):
    ...
    def clear(self):
        """移除所有没有exp属性的key"""
        for key in [key for key in self._key_nodes if not getattr(key, "exp", None)]:
            self.remove_key(key)
...
def on_message(*conditions, exp=None, debug=False, pre_hook=None, callback=None, register_help=None):
    ...
    def decorator(func: Callable):
        ...
        if (ann := get_type_hints(get_true_func(func)).get("event")) and (ann := ann.__pydantic_generic_metadata__.get("args")):
...
async def process_message(event: Message, ignore_prefix=False):
    ...
    for k in tuple(_message_handlers):
        ...
        for expr, func, token, hook in (v := _message_handlers[k]):
            if ignore_prefix:
...
async def process_notice(event: Notice):
    ...
    for k in tuple(_notice_handlers):
        ...
        for expr, func, token, _ in (v := _notice_handlers[k]):
            kwargs = {}
            if e:
                kwargs["event"] = event
            if l:
...
async def process_request(event: Request):
    ...
    for k in tuple(_request_handlers):
        e, l = "event" in k, "localizer" in k
        for expr, func, token, _ in (v := _request_handlers[k]):
            kwargs = {}
            if e:
                kwargs["event"] = event
            if l:
...
DEFAULT_SELECTOR_STRATEGY = cfg.register(
