...
USER_CACHE = LRUCache(cfg.get_config("user_aha_id", 32768, _("identity.user.cache.cfg_comment"), "cache"))
GROUP_CACHE = LRUCache(cfg.get_config("group_aha_id", 256, _("identity.group.cache.cfg_comment"), "cache"))
...
# region 用户
@overload
async def user2aha_id(platform: str, user_id: str, *, session=None) -> int: ...

@overload
async def user2aha_id(user_id: str, *, session=None) -> int: ...

@overload
async def user2aha_id(*, session=None) -> int: ...
...
async def user2aha_id(arg1=None, arg2=None, session=None):
    ...
    try:
        result: int = await session.scalar(select(AhaUser.aha_id).where(AhaUser.platform == platform, AhaUser.user_id == user_id))
        if result:
            return result
...
async def aha_id2user(aha_id: int) -> list[tuple[str, str]]:
    """根据 Aha ID 反向查找用户"""
    async with db_session_factory() as session:
        return (await session.execute(select(AhaUser.platform, AhaUser.user_id).where(AhaUser.aha_id == aha_id))).all()
...
async def aha_id2group(aha_id: int) -> list[tuple]:
    """根据 Aha ID 反向查找群组"""
    async with db_session_factory() as session:
        return (await session.scalars(select(AhaGroup.platform, AhaGroup.group_id).where(AhaGroup.aha_id == aha_id))).all()


# endregion
# region 批量
async def batch_aha_id2group(platform_user_pairs: Iterable[tuple[str, str]]):
    """批量获取用户 Aha ID"""
    return await asyncio.gather(*[user2aha_id(platform, user_id) for platform, user_id in platform_user_pairs])


async def batch_group2aha_id(platform_group_pairs: Iterable[tuple[str, str]]):
    """批量获取群组 Aha ID"""
    return await asyncio.gather(*[group2aha_id(platform, group_id) for platform, group_id in platform_group_pairs])
