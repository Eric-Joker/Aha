...
from tenacity import before_sleep, retry, retry_if_exception_type, stop_after_attempt, wait_exponential
...
class WebSocketClient(ClientTransport):
    ...
    async def open(
        ...
        self._retry_args = {
            ...
            "retry": retry_if_exception_type((WebSocketException, ConnectionError)),
            "before_sleep": before_sleep.before_sleep_log(self._logger, logging.WARNING),
            "reraise": True,
        }
        if retry_config:
