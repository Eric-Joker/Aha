...
async def init_load_mod(exclude: set = None):
    ...
    for finder, module_name, is_pkg in iter_modules(__path__):
        if module_name.startswith("DISABLED") or exclude and module_name in exclude:
            continue
...
async def reload_modules(disable: set[str] = None):
    ...
    _logger.info(_("module.reload.done") % (len(root_module_names) + await init_load_mod(exclude=root_module_shorter_names)))
