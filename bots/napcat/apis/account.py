...
class AccountAPI(BaseAccountAPI):
    ...
    async def set_avatar(self, call_id, file):
        return await self._call_api(
            call_id,
            "set_avatar",
            {"file": await self.prepare_file(file if isinstance(file, str) else file.file, self.transport.local_srv)},
        )
    ...
    async def _get_user_by_search(self, _, user_id, group_id=None) -> Stranger | Friend | GroupMemberInfo | None:
        if group_id:
            return next(
