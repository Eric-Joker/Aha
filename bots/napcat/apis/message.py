...
class MessageAPI(Utils, BaseMessageAPI):
    ...
    async def send_raw_msg(self: NapCat, call_id, *, user_id=None, group_id=None, data):
        if group_id:
            return await self.send_group_unescape_text(call_id, group_id, data)
        if user_id:
            return await self.send_private_unescape_text(call_id, user_id, data)
    ...
    async def get_forward_msg(self, call_id, message_id, raw_list=False):
        raw = (await self._call_api(call_id, "get_forward_msg", {"message_id": message_id}))["messages"]
        return raw if raw_list else await self.content2forward(message_id, raw)
