...
通过配置文件中的 `bots` 键配置与协议框架通讯，详见[已有适配器](./已有适配器/README.md)。
...
[配置文件](#配置与运行)的 `aha` 键下有 `default_group_list`、`default_user_list` 键值，表示全局默认的平台用户/群组黑白名单。

Aha 支持在[配置文件](#配置与运行)中为每个模块建立单独的黑白名单：
...
相关逻辑只运行在调用 `open` 方法时所在线程的异步事件循环，从其他线程调用时会自动转发到目标线程。
...
- 协议适配器逻辑应与其他 Aha 业务**完全分离**，不得直接引用[配置系统](./模块开发/统一配置系统.md)、[文件缓存](./模块开发/内置轮子与最佳实践/文件缓存.md)、[数据库](./模块开发/内置轮子与最佳实践/长效数据.md)、[计划任务](./模块开发/内置轮子与最佳实践/计划任务.md)、[统一 ID](./模块开发/跨平台统一%20ID.md)、[订阅事件](./模块开发/订阅与发布事件.md)等特性。若有需要参见 [Aha 服务调用](#aha-服务调用.md)。
- 发布的事件对象和 API 返回值应可 Pickle。

### 应具备的逻辑

- [注册为新的协议适配器](#创建适配器)
  - 在后续逻辑中可通过 `self.config` 字典获取配置。
  - `BaseBot` 会从配置中 pop 出 `start_server_command` 和 `stop_server_command` 存至 `self._start_server_comm` 和 `self._stop_server_comm` 属性。
...
- [将 API 调用转化为协议所需格式以调用聊天平台接口并返回值](#声明和调用-api)
  ...
  - 涉及文件时处理文件传输。
  - 将返回值封装为 [Pydantic Model](https://docs.pydantic.dev/latest/concepts/models/)。应优先采用 `models` 中已有的数据结构，可以继承并修改模型。[部分参考](./数据结构/API%20相关.md)
...
class Example(BaseBot):  # 继承 BaseBot 即注册为新的协议适配器，类名应全适配器唯一
...
成功连接（包括重新连接）时会调用 `_connect_cb` 方法。

连接断开会调用 `_disconnect_cb` 方法，`BaseBot._disconnect_cb` 内置上报 `DISCONNECT` 生命周期[元事件](./数据结构/事件对象.md#metaevent)的逻辑。
...
class Example(BaseBot):
    ...
    async def _listen_callback(self, data: bytes):
        self.logger.info(ssrjson.loads(data))
...
class Example(MessageAPI, BaseBot):
    ...
    async def _listen_callback(self, data: bytes):
        ...
        if echo := data.get("echo"):  # 视协议层通过 ws 发送 API 返回值。
            self._calls[echo].set_result(data)

    async def _call_api(self, echo, action, params=None, timeout=300) -> Any:
        future = self._calls[echo] = get_running_loop().create_future()
        await self.transport.invoke(ssrjson.dumps_to_bytes({"action": action, "params": params or {}, "echo": echo}))
        try:
...
print(gettext('hello'))  # 此时输出的是配置文件中 `aha.lang` 所指定语言的翻译，若没有翻译会尝试回退直至键名本身。
```

`core.i18n._()` 是 `core.i18n.gettext()` 的别名，返回 `LocalizedString` 对象，它继承自 `str`。

#### 2.2 通过[表达式](事件匹配表达式.md)匹配本地化键名动态获取本地化

对 `PM.message` 采用正则相关或相等/不等比较运算符时，若另一操作数为本地化字符串，则会匹配所有语言，创建第一个匹配成功的语言的翻译器并传递给回调的 `localizer` 关键字参数。
...
> 所有通过[注册回调装饰器工厂](./订阅与发布事件.md#注册回调函数)装饰的函数均可声明 `localizer` 关键字参数。
>
...
比如，若没有为 `es_ES` 提供翻译，但只要提供了 `es` 的翻译，也会被使用。
...
参数 `module` 指定从哪个 Python 模块的 `locales` 子目录中的本地化文件中查找，为 None 时代表框架根目录。
...
> 由于 Aha 可以同时对接多个协议服务，若多个协议适配器实例上报了同一平台的相似的事件则会自动去重。

### 事件类别

对于模块，共有 5 种公开事件类别和 3 种非公开事件类别。

| 类别 | 对模块公开 | [对象](../数据结构/事件对象.md) | 注册回调装饰器工厂 | 说明 |
| --- | --- | --- | --- | --- |
...
| EXTERNAL | ✅️ | [models.api.External](../数据结构/事件对象.md#external) | core.dispatcher.on_external | 非标准事件。提供将任意数据通过事件分发的方式。可用于 Web 请求，也可以作为模块间的触发器。 |
...
模块通过声明一个函数，并通过上述注册回调装饰器工厂装饰该函数以订阅事件。

对于标准事件，注册回调装饰器工厂的位置参数用于[基于 DSL（领域特定语言）的筛选方案](./事件匹配表达式.md)；当然，简单地通过正则的筛选并无不可。

事件分发器调用回调时会通过 `event` 关键字参数传递[事件对象](../数据结构/事件对象.md)实例。对于 External 事件，可通过 `data` 关键字参数直接获取事件对象的 `data` 属性。

> 仅可装饰异步函数。不得装饰非 Python 实现的函数，因为无法获取参数信息。

#### 一次性回调

通过声明注册回调装饰器工厂的 `exp`（过期时间）参数实现，当成功触发一次后，回调将被自动注销。
...
@on_message("注册一次性回调", threadable=False)
...
若[配置文件](./统一配置系统.md)中 `aha.execution_mode` 被设置为 thread，则可能会由子线程调用回调；若逻辑不保证线程安全，则应声明注册回调装饰器工厂的 `threadable` 参数为 `False`，届时只会在主线程执行回调。
...
`process_external` 等函数线程安全（会将调用请求转发至主线程）。

```python
from functools import partial
from core.dispatcher import on_start, process_external
from models.api import External

on_start(partial(process_external, External(key="key", data=object())))
```
...
>
> 黑魔法特性包括非优化路径的 `Apply` 运算符、非优化路径的[属性穿透](#属性穿透)、[pre_hook](./订阅与发布事件.md#预处理匹配的消息内容) 。

本系统除[进阶教程](#进阶教程)所述特性外，在设计使用场景下可视为线程安全。

### 字段

字段代表从事件中提取到的特定值，例如消息文本、事件类型等。
...
每个字段都指定了从[事件对象](../数据结构/事件对象.md)提取对应值的逻辑，其返回值类型决定了可进行的运算。
...
| `PM.limit` | bool | Aha 内置的以平台用户为单位的限速特性。提取值为限速触发者的状态，为 True 代表状态正常未限制。若总匹配条件较为宽泛应显式声明 `Plimit == False` | -999 |
...
- **`PM.limit`**：其提取器是计数、验证一体的。当 `PM.limit == False` 时会直接跳过提取器逻辑并将该二元表达式评估结果设为 True。

- **`PM.command`**：将消息以空格分割作命令。
  - 基本用法：进行 `==`、`StartsWith` 和 `IsOnly` 相关的运算时，第二操作数可指定为期望的参数列表，列表元素可以是：
    - 字符串：要求对应位置的参数严格相等。
    - 类型注解：比如 `int`、[`Image`](../数据结构/消息序列与消息段.md#image)。要求对应位置的参数验证通过，且会将验证后的值组成 Sequence 通过 `args` 关键字参数**传递给回调函数**。  
    `StartsWith` 匹配后剩余位置的参数也会依次存入 `args` 列表。
  - 示例，假设去除前缀后的消息内容为 `"echo hello world 1"`：
    ```python
    PM.command == ["echo", str, Text, int]  # args = ["hello", "world", 1]，Text 会被视作 str 作为类型注解
    PM.command == ["echo", "hello", "world" "1"]  # args = []
    PM.command.startswith(["echo"])  # args = ["hello", "world", "1"]
    ```
  - 提取时，先从消息中移除[前缀](./内置轮子与最佳实践/消息前缀.md)，然后遍历消息链：  
    若为 [Text 消息段](../数据结构/消息序列与消息段.md#text)则将**其内容**按空格分割为多个字符串参数。若含换行，第一行最后一个参数与换行后的所有内容合并为一个参数（保留换行）。  
    非 Text 的[消息段](../数据结构/消息序列与消息段.md)作为独立参数保留。

### 运算符

运算符需依托于表达式实例调用，字段就是表达式实例：`Pmsg == "value"`。
...
| `.validateby()` | `ValidateBy` | 对左侧对象使用类型注解进行验证，返回验证后类型的对象。 | `Pmsg_chain.validateby(list[At])` |
...
> 由于可能会修改[事件对象](../数据结构/事件对象.md)实例的内容，出于回调间数据隔离的需求，使用 `Apply` 运算符会导致在事件匹配前就提前对[事件对象](../数据结构/事件对象.md)实例进行以回调为单位的深拷贝，这会降低性能。  
> 以下情况会优先采用优化路径，届时不会进行提前深拷贝：
> - 要被应用的函数为 builtin 中的 `abs`、`aiter`、`ascii`、`bin`、`callable`、`chr`、`dir`、`format`、`hash`、`hex`、`id`、`iter`、`len`、`oct`、`ord`、`repr`、`round`、`vars`。
> - 要被应用的类型为 builtin 中的 `int`、`float`、`complex`、`str`、`type`、`range`、`slice`、`memoryview`、`super`、`reversed` 或它们的子类型。

> <small style="color: gray;">由于存在评估优先级的特性存在，我想不到好的方案以实现变量功能。</small>

#### 属性穿透
表达式实例重载了 `__getattr__` 和 `__call__`。因此，评估时表达式会将上述未声明的公共属性/方法、下述魔术方法的调用穿透到提取值上。
...
> 不会额外拦截私有属性与方法，但请注意与表达式实例自有属性冲突的风险。

由于可能会修改[事件对象](../数据结构/事件对象.md)实例的内容，出于回调间数据隔离的需求，使用本特性会导致在事件匹配前就提前对[事件对象](../数据结构/事件对象.md)实例进行以回调为单位的深拷贝，这会降低性能。

这些魔术方法的语义不会修改原对象，调用它们时会优先采用优化路径，届时不会进行提前深拷贝：`__lt__`、`__le__`、`__gt__`、`__ge__`、`__getitem__`、`__add__`、`__sub__`、`__mul__`、`__matmul__`、`__truediv__`、`__floordiv__`、`__mod__`、`__pow__`、`__divmod__`、`__radd__`、`__rsub__`。

### 默认表达式

通过装饰器注册回调时，系统会自动为某些字段补充默认表达式，并与通过[注册回调装饰器工厂](./订阅与发布事件.md#注册回调函数)的参数直接声明的表达式进行逻辑与合并。当然，若显式声明了这些字段则不会添加对应的默认表达式。
...
> 如下阐述所有 Aha 原生字段的相关特性，模块的自定义字段可能存在类似特性。

#### 简写为非表达式对象

可以尝试在[注册回调装饰器工厂](./订阅与发布事件.md#注册回调函数)的位置参数传入非表达式的 Python 对象作为参数，系统会尝试依据类型自动将其转换为表达式：

- **str**  
  - 若传递给 `on_message` 则转化为 `PM.message.fullmatch(re.compile(str, re.I))`
  - 否则，若作为顶层参数依次出现，则
    - 第一个字符串 → `PM.type_ == EventType(str1)`
    - 第二个（若有）→ `PM.sub_type == EventSubType(str2)`。
...
@on_notice("group_increase", "approve")  # 等价于 (PM.type_ == NoticeEventType.GROUP_INCREASE) & (PM.sub_type == NoticeEventType.APPROVE)
```
...
- **正则运算符**（`match`、`fullmatch`、`search`）：若指定对象是 str，会自动编译为**忽略大小写**的正则表达式。

#### 对[消息链](../数据结构/消息序列与消息段.md#messagechain)的类型注解支持

通过[注册回调装饰器工厂](./订阅与发布事件.md#注册回调函数)的参数直接声明的表达式中**未使用** `PM.message`、`PM.message_chain`、`PM.command` 字段，且被装饰函数的 `event` 参数存在指定了 [`Message`](../数据结构/事件对象.md#message) 泛型的类型注解，则会基于此泛型注解生成 `Pmsg_chain.validateby(MessageChain[T])` 表达式并与直接声明的表达式进行逻辑与合并。
...
| `PM.notice` | `Pnotice`、`PM.type_` |
| `PM.request` | `Prequest`、`PM.type_` |
| `PM.meta` | `Pmeta`、`PM.type_` |
| `PM.sub_type` | `Psub_type` |
| `PM.gid` | `Pgid` |
| `PM.uid` | `Puid` |
| `PM.group` | `Pgroup` |
| `PM.user` | `Puser` |
...
若[配置文件](./统一配置系统.md)中 `aha.debug` 为 `true` 则会进入调试模式：
 - 在[注册回调装饰器工厂](./订阅与发布事件.md#注册回调函数)将关键字参数 `debug` 传为 `True`，则在表达式评估时会打印每个二元运算符操作数与匹配结果。
    ```python
...
- **主要流程**：向[注册回调装饰器工厂](./订阅与发布事件.md#注册回调函数)传入表达式后，会构造出由表达式实例组成的树，在每次接收到新的事件时通过 `core.expr.evaluate` 函数进行递归评估得出**评估值**。
- **表达式实例**：继承自 `Expr` 的类的实例。**字段**与**二元/逻辑表达式实例**都是表达式实例。
- **字段**：`core.expr.FieldClause` 的实例，被评估时通过提取器获取**评估值**。其 `field` 参数是 [`Field 描述符`](#field-描述符)，定义了提取器、默认表达式、优先级等字段元信息。
- **二元表达式**：继承自 `BinaryExpr` 的类，如 `Equal`、`In` 等，通过 `_evaluate_logic` 方法得出**评估值**。
- **逻辑表达式**：`And`、`Or`、`Not`，实例具有 `clause` 属性以存储子表达式。

#### 注册 `PM.validated` 的提取器

`PM.validated` 的提取器不是原生静态定义的，需要模块通过 `register_extractor` 装饰器注册。
...
| key_func | Callable[[type[BinaryExpr], Any, [BaseEvent](../数据结构/事件对象.md#baseevent)], Hashable] | 生成缓存键的函数。第二个参数为另一操作数。推荐返回为 `hash()` 的结果以释放内存。 |
...
| skip_cache | Callable[[type[BinaryExpr], Any, BaseEvent], bool] | 决定是否执行缓存相关逻辑。第二个参数为另一操作数。返回为 `True` 时不执行。 |
| contextvars | Iterable[str] | 同时缓存评估结束后指定的 ContextVar，缓存命中时应用。 |

> 由于缓存逻辑略复杂，应只有当提取器或运算符确实很耗时时再考虑使用缓存，或通过 `skip_cache` 该跳过跳过。

#### 新增运算符

继承 `BinaryExpr` 以定义一个新的二元表达式类并实现 `_evaluate_logic` 方法：

其除了 `self` 还具有**两个位置参数**，分别是左操作数（一般是字段）的**评估值**与右操作数的**评估值**。

返回的值即为本二元表达式的实例的**评估值**。

```python
from core.expr import BinaryExpr, Pmsg

class Example(BinaryExpr):
    def _evaluate_logic(self, left, right):
        return left == "hello" and right == "world"

# 使用方式参考
on_message(Example(Pmsg, "world"))
```

> 构建表达式树中的二元表达式时，会优先将字段调整为左操作数，次之会优先将表达式对象调整为左操作数。
...
Aha 启动时加载完所有模块后，若发现具有新的配置项则会终止进程并提示用户修改配置。
...
**键独有限制**：
- 类型只可为 `str`、`int`、`float`、`datetime.date`、`NoneType`、`Sequence`、`anyio.Path` 或它们的子类。
- `Sequence` 会被视为 `tuple`。
...
**值独有限制**：
- 类型只可为 `str`、`int`、`float`、`datetime.date`、`NoneType`、`Sequence`、`Mapping`、`Set`、`anyio.Path`、[`Option`](#coreconfigoption)、[`IndexedBotUser`](#coreconfigindexedbotuser--coreconfigindexedbotgroup)、 [`IndexedBotGroup`](#coreconfigindexedbotuser--coreconfigindexedbotgroup) 、它们的子类或 dataclass、[attrs](https://github.com/python-attrs/attrs)、[Pydantic Model](https://docs.pydantic.dev/latest/concepts/models/)。

**键和值共有的限制**：
- 所有容器长度必须大于等于 1。
- `Set` 的元素要么均为 `str`、`int`、`float`、`datetime.date`、`NoneType` 及其子类，要么所有元素类型相同且只可为 `anyio.Path`、`Sequence`、`Mapping`、`Set`、[`IndexedBotUser`](#coreconfigindexedbotuser--coreconfigindexedbotgroup)、 [`IndexedBotGroup`](#coreconfigindexedbotuser--coreconfigindexedbotgroup) 、它们的子类和或dataclass、[attrs](https://github.com/python-attrs/attrs)、[Pydantic Model](https://docs.pydantic.dev/latest/concepts/models/)。前者时不会进行类型验证，后者时会将所有元素视为同一个类型。
...
大部分事件相关的对象都声明在 `models.api` 中，所有事件对象都是 [`BaseEvent`](#baseevent) 的子类，是 [Pydantic Model](https://docs.pydantic.dev/latest/concepts/models/)。
...
| 属性/方法 | 类型 | 说明 |
| --- | --- | --- |
| event_type | EventType(enum) \| None | 该事件类别的主类型。 |
| sub_type | EventSubType(enum) \| None | 该事件类别的子类型。 |
| self_id | str | 协议服务所登陆的账号 ID。 |
| time | aware datetime | 时间。 |
...
| message | [MessageChain](./消息序列与消息段.md#消息序列) | 消息内容。别名 `msg`。 |
| message_str | str | 通过 [Aha 码](../模块开发/Aha%20码.md)内联的消息内容。别名 `msg_str`。 |
| get_msg_inline() | [InlineStr](../模块开发/内置轮子与最佳实践/零碎%20utils.md#inlinestr) | 获取对象内联字符串。 |
| sender | [MessageSender](#messagesender) | 发送者的更多基础信息。 |
| user_id | str | 发送者平台 ID。 |
| group_id | str \| None | 平台群组 ID。 |
...
「加好友」或「加群」请求。[示例](../模块开发/基本教学.md#处理-request-事件加群申请等)
...
承载群或群成员状态变更、消息状态变更、Poke等通知。[示例](../模块开发/基本教学.md#处理-notice-事件群成员变更等)
...
| user_id | str | ALL | 操作相关平台用户 ID。 |
| group_id | str | group_admin、group_card、group_decrease、group_increase、group_upload、group_recall、group_ban、essence、reaction、notify.title、notify.group_name、notify.poke | 操作相关平台群组 ID。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| target_id | str \| None | 私聊平台用户 ID。 |

### External

| 属性 | 类型 | 说明 |
| --- | --- | --- |
| key | str | 标识，主要用于 on_external 的订阅。 |
| data | Picklable | 数据。 |
| lang | str \| None | 为回调推荐应使用的语言。 |
...
Aha 使用**消息段（MsgSeg）** 作为消息的基本组成单元，多个消息段按顺序组合成**消息序列（MessageChain）**。
...
是 [Pydantic Model](https://docs.pydantic.dev/latest/concepts/models/)。每个消息段代表一种特定类型的消息内容，如纯文本、图片、@等。所有消息段均继承自 `MsgSeg`，并拥有以下通用特性：
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| group_id | str \| None | 群文件所属平台群组 ID。 |
...
添加一个节点。可指定 `user_id` 和 `nickname` 参数，未指定时自动从 `bot_id` 参数[获取](../模块开发/向协议服务请求/Account%20有关%20API.md#get_login_info)，未指定 `bot_id` 时采用与当前事件上下文相同的。
...
提供的所有缓存器本身均非线程安全，但 [`async_cached`](#async_cached) 通过额外逻辑保证了线程安全。

### `@async_cached()`

`core.cache.async_cached` 是一个逻辑类似 `functools.lru_cache` 的函数，返回仅适用于异步的装饰器。该装饰器逻辑耦合、功能不全，但是能用。

若有需要可以尝试 [cachetools-async](https://github.com/imnotjames/cachetools-async)。
...
| lock | ✅️ | `aiologic.Lock` 或其子类的实例。若不提供则生成仅用于这一个被装饰函数的。 |
...
| 参数名 | 说明 |
| --- | --- |
| cache_read | bool 为 `False` 时本次调用不查询缓存。 |
| cache_key | 指定本次调用的缓存键生成器。 |

### 示例

```python
from core.cache import MemLRUCache, async_cached
from core.config import cfg
...
@async_cached(MemLRUCache(CACHE_SIZE))
...
计划任务相关逻辑只运行在主线程的异步事件循环，从其他线程调用时会自动转发到主线程。

> 指定 ID 和 metadata 时请尽可能避免不同模块之间的<small style="color: gray;">~~默契~~</small>冲突。

### 使用与示例

封装实例为 `services.apscheduler.sched`。使用方法与 [`AsyncScheduler`](https://apscheduler.readthedocs.io/en/master/api.html#apscheduler.AsyncScheduler) 实例大致一致，但也有所调整。
...
通过类方法 `from_iterable` 将可迭代对象的**所有元素拼接成一个字符串**，并为其中**非字符串且非 [`Text`](../../数据结构/消息序列与消息段.md#text) 实例**的对象分别**随机分配为一个字符**。分配的字符可能为未分配码点或 [PUA](https://zh.wikipedia.org/wiki/%E7%A7%81%E4%BA%BA%E4%BD%BF%E7%94%A8%E5%8C%BA)，会规避原字符串中已有字符。
...
async with cache_file_sessionmaker(file_ext=".txt") as session:  # 文件名采用内容哈希（xxh3 128）或随机，可通过 file_ext 为生成的路径添加扩展名
    path: Path = await session.register(300, b'hello world')  # 过期时间 300 秒，但自动清理时机跟随配置文件的 cache.file_cache.cleanup_cron
...
Aha 声明支持 SQLite 与 PostgreSQL。

开发时若使用重载特性建议将 Table 定义在模块目录的 `database.py` 文件或 `database` 目录下，这两个地方不会被重载。

[Sqlalchemy](https://www.sqlalchemy.org/) 在设计使用方式下是线程安全的。
...
4. 键只能是字符串，值需可被 pickle。
5. 完全线程不安全，加锁也没用。
6. `SimpleStore` 并非 `dict` 的子类。
...
# File: 模块开发/内置轮子与最佳实践/Playwright.md
