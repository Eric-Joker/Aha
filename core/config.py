...
# from utils.misc import MODULE_PATTERN
from utils.misc import caller_module, get_item_by_index
...
class Config(metaclass=SingletonMeta):
    ...
    def __init__(self):
        ...
        # self._lock = Lock()
        self._config_file = aioPath(f"config.{getenv("BOT_ENV", "dev")}.yml")
        self._loaded = False
    ...
    def __getitem__(self, key):
        return self.register(key, module=caller_module())

    def __setitem__(self, key: str, value):
        self.set(key, value, module=caller_module())

    def get(self, key, default=None, module=None):
        try:
            return self.register(key, module=module or caller_module())
        except KeyError:
            return default
    ...
    def _loads(cls, obj, key, type_map: dict[str, type | tuple]):
        ...
        try:
            ...
            raise ValueError("Unable to convert the configuration value to the required type.") from e
    ...
    def _dumps(cls, obj, parent: OptionCommentedMap | OptionCommentedSeq, index, type_map: dict):
        ...
        if isinstance(obj, Mapping):
            ...
            for key, value in obj.items():
                if not isinstance(key, str):
                    raise TypeError(_("config.type_convert.key_type_err") % key.__class__.__name__)
        ...
        if isinstance(obj, aioPath):
            ...
            return str(obj)
        raise TypeError(_("config.type_convert.unsuported") % obj.__class__.__name__)
    ...
    async def register_async[T: Any | Option](self, key: str, default: T = None, comment: str = None, module: str = None) -> T:
        async with self._lock:
            return self.register(key, default, comment, module)
    ...
    def register[T: Any | Option](self, key: str, default: T = None, comment: str = None, module: str = None) -> T:
        ...
        return self._register(key, default, comment, module)
    """

    def register[T: Any | Option](self, key, default: T = None, comment=None, module=None) -> T:
        ...
        if (storage_module := module or caller_module()) is None:
            return
        ...
        for mod in {"aha", storage_module}:
            if (data := self._data.get(mod)) and key in data:
                ...
                value = self._loads(value := data[key], key, self._default_types[mod])
    ...
    def clean(self):
        self._yaml = self._safe_yaml = self._old_data = self._data = self._config_file = self._default_types = self._loaded = (
            self._default_used
        ) = self._modified = self._group_blacklist = self._user_blacklist = self._group_whitelist = self._user_whitelist = None
    ...
    def message_prefix(self):
        return self.get("message_prefix", module="aha")
    ...
    @property
    def green_db(self):
        return self.get("green_db", module="aha")
    ...
    @property
    def point_feat(self):
        return self.register("enable", False, _("config.comment.point_feat"), "point")

    @property
    def low_memory(self):
        return self.get("low_memory", module="aha")
    ...
    @property
    def playwright(self):
        return self.register("playwright", False, _("config.comment.playwright"), "aha")
            ...
            self._data["bots"] = self._old_data["bots"] = [
                {
                    "NapCat": {
                        ...
                        "retry_config": {"wait_exponential": (1, 30, 2, 1)},
                    }
                }
            ]
...
"""
cfg.register("lang", os.environ.get("LANG").split(".")[0], "Default language.", "aha")
cfg.register("console_level", Option(getLevelNamesMapping(), "INFO"), _("config.comment.console_log_level"), "log")
cfg.register("file_level", Option(getLevelNamesMapping(), "AHA_DEBUG"), _("config.comment.file_log_level"), "log")
cfg.register("max_files", 5, _("config.comment.max_log_files"), "log")
cfg.register("max_size", "16MiB", _("config.comment.log_file_max_size"), "log")


def init_base_cfgs():
    cfg.register("super", (User("QQ", "114514"),), "Super user ID.", "aha")
    cfg.register("message_prefix", "~", _("config.comment.message_prefix"), "aha")
    cfg.register("database", "sqlite+aiosqlite:///data.db", _("config.comment.database"), "aha")
    cfg.register("green_db", "sqlite:///data.db", _("config.comment.green_db"), "aha")
    cfg.register("cache_conv", False, _("config.comment.cache_conv"), "aha")
    cfg.register("low_memory", False, _("config.comment.low_memory"), "aha")
    cfg.register("base64_buffer", 1919810, _("config.comment.base64_buffer"), "aha")
    cfg.register("file_msg_ttl", 3600, _("config.comment.file_msg_ttl"), "cache")
    cfg.register("event", {"size": "16MiB", "ttl": 86400}, _("config.comment.event_cache"), "cache")
    cfg.register(
