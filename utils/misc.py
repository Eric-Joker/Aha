...
def get_arg_count(func: Callable[..., Any]):
    ...
    return func.__code__.co_argcount - 1 if is_instance_method(func) else func.__code__.co_argcount


def get_arg_names(func: Callable[..., Any]):
    """获取函数的所有参数名，不包含可变参数和self/cls"""
    names = list((all_arg_names := (code := func.__code__).co_varnames)[: (arg_count := code.co_argcount)])
