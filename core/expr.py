...
from utils.misc import AHA_MODULE_PATTERN, find_first_instance, is_prefix, is_suffix
...
if EXPR_DEBUG := cfg.register("expr_debug", False, _("expr.debug.cfg_comment"), "aha"):
...
def _groups_default_factory(f: Expr):
    from .router import current_module
        ...
        for i, v in enumerate(value):
            if not isinstance(v, str):
                if args := getattr(v, "__args__", None):
                    v.__args__ = tuple(Text if a is str else a for a in args)
...
def remove_msg_seq_prefix(msg: MessageChain):
    from .router import current_event, current_module, cugp
...
def _has_msg_prefix(event: Message):
    from .router import current_module, cugp
...
def field_exists(expr: Expr | Any, field: FieldClause | Iterable[FieldClause], binary_expr: type[BinaryExpr] = None):
    """检查表达式中是否存在指定字段
    ...
    if isinstance(expr, BoolExpr):
        return any(field_exists(c, field, binary_expr) for c in expr.clauses)
    elif isinstance(expr, Not):
        return field_exists(expr.clause, field, binary_expr)
    elif isinstance(expr, BinaryExpr):
        return (
            binary_expr
            and (expr.left is field or isinstance(field, Iterable) and any(expr.left is x for x in field))
            and isinstance(expr, binary_expr)
            or field_exists(expr.left, field, binary_expr)
            or field_exists(expr.right, field, binary_expr)
        )
