...
from core.expr import EXPR_DEBUG
...
from utils.misc import FULL_AHA_MODULE_PATTERN, caller_aha_module, get_arg_names, get_item_by_index, get_true_func

from .config import Option, cfg
from .expr import PM, And, ApplyTo, Expr, build_cond, cprmc, cprms, evaluate, field_exists, remove_msg_seq_prefix
    ...
    def decorator(func: Callable):
        ...
        if (ann := get_type_hints(get_true_func(func)).get("event")) and (ann := ann.__pydantic_generic_metadata__.get("args")):
            if not field_exists(conditions, (PM.msg, PM.command, PM.msg_chain)):
                conditions = And(conditions, PM.msg_chain.validateby(TypeAdapter(MessageChain[ann[0]])))
                conditions.exp = exp
                if EXPR_DEBUG:
                    conditions.debug = debug
        ...
        _message_handlers[frozenset(args)].add(
            conditions,
            func,
            ExprPoolAttach(module, pre_hook, field_exists(conditions, PM.message_chain, ApplyTo), register_help is not None),
        )
        return func
...
def on_notice(*conditions, exp=None, debug=False, callback=None):
    ...
    def decorator(func: Callable):
        ...
        _notice_handlers[frozenset(args)].add(
            build_cond(conditions, EventCategory.NOTICE, exp, debug), func, ExprPoolAttach(module)
        )
        return func
...
def on_request(*conditions, exp=None, debug=False, callback=None):
    ...
    def decorator(func: Callable):
        ...
        _request_handlers[frozenset(args)].add(
            build_cond(conditions, EventCategory.REQUEST, exp, debug), func, ExprPoolAttach(module)
        )
        return func
...
def on_meta(*conditions, exp=None, debug=False, callback=None):
    ...
    def decorator(func: Callable):
        ...
        _meta_handlers[frozenset(args)].add(
            build_cond(conditions, EventCategory.META, exp, debug), func, ExprPoolAttach(module)
        )
        return func
...
async def process_message(event: Message, ignore_prefix=False):
    async def evaluate_and_route(expr, token, pool, attach: ExprPoolAttach, e, m, a, l):
        nonlocal event
        copied = False
        ...
        if await evaluate(event, expr, token, pool):
            ...
            if e:
                ...
                kwargs["event"] = event if copied else event.model_copy(deep=True)
            if m:
    ...
    for k in tuple(_message_handlers):
        ...
        for expr, func, token, attach in (pool := _message_handlers[k]):
            ...
            await create_task(evaluate_and_route(expr, token, pool, attach or None, e, m, a, l))
