...
class Config(metaclass=SingletonMeta):
    ...
    def get_config(self, key: str, default=None, module: str = None, comment: str = None):
        ...
        for mod in ["aha", storage_module]:
            if mod in self._data and key in self._data[mod]:
                ...
                target_type = self._default_types.get(mod, {}).get(key) if default is None else type(default)
        ...
        if isinstance(default, Iterable) and not isinstance(default, (tuple, frozenset, str, bytes)):
    ...
    def enable_fastapi(self) -> bool:
        return self.get_config("enable_fastapi", True, "aha")
