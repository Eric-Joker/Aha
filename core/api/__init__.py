...
class APIMeta(type):
    def __new__(cls, name, bases, namespace, **kwargs):
        ...
        for attr_name, attr_value in namespace.items():
            if not attr_name.startswith("__") and type(attr_value) is FunctionType:
        ...
        for base in bases:
            for attr_name in dir(base):
                if not attr_name.startswith("__"):
                    if type(attr_value := getattr(base, attr_name)) is FunctionType and attr_name not in methods_to_wrap:
                        methods_to_wrap[attr_name] = attr_value
