...
from collections.abc import Callable, Container, Hashable, Iterable, KeysView, Mapping, Sequence, ValuesView, MutableSequence
...
from typing import TYPE_CHECKING, Any, Hashable, Literal, NoReturn, SupportsIndex, TypedDict, Unpack, _Final, _UnionGenericAlias
...
from utils.misc import MODULE_PATTERN, caller_module, is_prefix, is_suffix
from utils.string import convert_text
from utils.typekit import parse_size
#from utils.typekit import Strable

from .cache import LRUCache, MemLRUCache, async_cached, hashkey
...
if DEBUG := cfg.get_config("expr_debug", False, _("expr.debug.cfg_comment"), "aha"):
    _current_debug = ContextVar("expr_debug", default=None)
...
class Expr[Result]:
    ...
    def __hash__(self):
        return hash(
            ((self.__class__, exp) if (exp := getattr(self, "exp", None)) else (self.__class__,))
            + tuple(
    ...
    def has_field(self, field: FieldClause):
        """表达式中是否包含指定字段"""
        return check_field_exists(self, field)
    ...
    def match(self, obj):
        """只有 `self` 为 `PM.message` 时，`obj` 才支持为 `Strable`，届时会通过 `re.I | re.M` 编译。"""
        # if self is PM.message and isinstance(obj, Strable) or isinstance(obj, re.Pattern):
...
class FieldClause[Result](Expr[Result]):
    """字段访问表达式。只能由 PatternMatcherMeta 创建该类的实例！"""
    ...
    def __repr__(self):
        if exp := getattr(self, "exp", None):
            return f"Field.{self.name}(exp={strftime("%Y-%m-%d %H:%M:%S", localtime(exp))}, PRI={self.priority})"
...
class Field:
    ...
    def __init__(
        ...
        Args:
            ...
            operand_types: 注册类型与其与字段的的二元运算符。用于当用户传入了一个非表达式的对象时，自动将其转换为表达式。
            ...
            redirect: 重定向到其他字段。
        """
        if not extractor and not requires_extractor and not redirect:
            raise ValueError("Field requires an extractor or requires_extractor=True")
        ...
        self.binary_semantics = binary_semantics
        self.operand_types = operand_types
        self.cache = cache
        self.skip_default_on_meta = skip_default_on_meta
        ...
        for k, v in tuple(namespace.items()):
            if isinstance(v, Field):
                if v.redirect:
                    # 重定向
                    try:
                        fields[k] = fields[v.redirect]
                        namespace[k] = namespace[v.redirect]
                    except KeyError as e:
                        raise AttributeError(_("expr.pattern_matcher.redirect.error") % {"from": k, "to": v.redirect}) from e
                else:
                    fields[k] = v
                    namespace[k] = fc = FieldClause(k, v)

                # 注册具有默认二元表达式的类型
                if v.operand_types:
                    for types, operand in v.operand_types.items():
                        if isinstance(types, Iterable):
                            for type_ in types:
                                _registed_operand_types[type_] = partial(operand, fc)
                        else:
                            _registed_operand_types[types] = partial(operand, fc)
...
class RawCondition(Expr[NoReturn]):
    ...
    def __repr__(self):
        if exp := getattr(self, "exp", None):
            return f"Raw.{self.value!r}(exp={strftime("%Y-%m-%d %H:%M:%S", localtime(exp))}, PRI={self.priority})"
...
class BinaryExpr[Left, Right, Result](Expr[Result]):
    ...
    def __repr__(self):
        if exp := getattr(self, "exp", None):
            return f"{self.left!r} {self.__class__.__name__} {self.right!r}(exp={strftime("%Y-%m-%d %H:%M:%S", localtime(exp))}, PRI={self.priority})"
        return f"{self.left!r} {self.__class__.__name__} {self.right!r}(PRI={self.priority})"

    async def evaluate(self, msg) -> Result:
        # 覆盖
        if type(self.left) is FieldClause and (result := self.left.field.overrides) and (result := result.get(self.right)):
    ...
    async def _evaluate_wrapper(self, *, left, right, msg) -> tuple[Any, dict[ContextVar, Any]]:
        ...
        if type(self._left_val) is AlwaysTrue or type(self._right_val) is AlwaysTrue:
        ...
        self._left_val = self._right_val = None
        return result, (
            {(obj := getattr(router, v)): obj.get() for v in self._cache_config.contextvars} if self._cache_config else {}
        )
...
class BoolExpr(Expr[bool]):
    ...
    def __repr__(self):
        if exp := getattr(self, "exp", None):
            return f"{self.__class__.__name__}({", ".join(repr(c) for c in self.clauses)}, exp={strftime("%Y-%m-%d %H:%M:%S", localtime(exp))})"
...
def _command_evaluate(left, right):
    from .router import current_args

    args = []
    for i, v in enumerate(right):
        if v != (arg := left[i]):
            if type(v) is TypeAdapter:
                with suppress(ValidationError):
                    v.validate_python(arg)
                    args.append(arg)
                    continue
            return False
    current_args.set(tuple(args))
    return True
...
class Equal(BinaryExpr[Any, Any | LocalizedString, bool]):
    async def _evaluate_logic(self):
        if self.left is PM.command:
            ...
            from .router import current_lang
...
class NotEqual(BinaryExpr[Any, Any | LocalizedString, bool]):
    async def _evaluate_logic(self):
        if self.left is PM.command:
            return (
                (not _command_evaluate(self._left_val, self._right_val))
                if len(self._left_val) == len(self._right_val)
                else True
            )
    ...
    async def _evaluate_logic(self):
        if self.left is PM.command:
            if len(self._left_val) >= (rl := len(self._right_val)) and _command_evaluate(self._left_val, self._right_val):
                ...
                current_args.set(tuple(self._left_val[rl:]))
                return True
            return False
...
class SingletonOf(BinaryExpr[Sequence, Any, bool]):
    async def _evaluate_logic(self):
        if len(self._left_val) != 1:
            return False
        if self.left is PM.command and type(self._right_val) is TypeAdapter:
            from .router import current_args
...
class Match(BinaryExpr[str, re.Pattern, re.Match | None]):
    async def _evaluate_logic(self):
        from .router import current_lang, current_match
...
class Not(Expr):
    ...
    def __repr__(self):
        if exp := getattr(self, "exp", None):
            return f"Not({repr(self.clause)}, expiration={strftime("%Y-%m-%d %H:%M:%S", localtime(exp))}, proiority={self.priority})"
        return f"Not({repr(self.clause)}, proiority={self.priority})"
...
# region 默认表达式工厂
current_module = ContextVar("current_module")


def _groups_default_factory(f: FieldClause):
    if l := cfg.get_group_whitelist(m := current_module.get()):
        return f.in_(l)
    return f.notin_(l) if (l := cfg.get_group_blacklist(m)) else None


def _users_default_factory(f: FieldClause):
    if l := cfg.get_user_whitelist(m := current_module.get()):
        return f.in_(l)
    return f.notin_(l) if (l := cfg.get_user_blacklist(m)) else None


# endregion
# region 右端值转换器
def _convert_pattern_rhs(value, operand, category):
    assert category is EventCategory.CHAT
    value = str(value)
    if issubclass(operand, (Match, FullMatch, Search)):
...
def _convert_chain_rhs(value, operand, category):
    assert category is EventCategory.CHAT
    if issubclass(operand, ValidateBy):
        return value if isinstance(value, TypeAdapter) else TypeAdapter(value)
    return value
...
# endregion
# region 限速
LIMIT = cfg.get_config("limit", 3, _("expr.fields.limit.cfg_comment"), "aha")
...
async def _check_rate_limit(msg: BaseEvent):
    """被限速 => False，正常状态 => True"""
    if not LIMIT or not hasattr(msg, "user_id") or await _is_admin(msg):
        return True
    current_time = time()
...
# endregion
# region 消息处理
cprms: ContextVar[str] = ContextVar("_cprms", default=None)  # current prefix removed str msg
cprmc: ContextVar[MessageChain] = ContextVar("_cprmc", default=None)  # current prefix removed msg chain


def remove_str_msg_prefix(msg: str):
    from .router import current_event

    event = current_event.get()
    if (cache := event.message_str is msg) and (mod := cprms.get()):
        return mod

    if msg:
        seq = []
        if cfg.message_prefix:
            seq.extend((cfg.message_prefix, convert_text(cfg.message_prefix)))
        if event.self_id:
            seq.append(f"[Aha:at,user_id={event.self_id}]")
        for prefix in seq:
...
def remove_msg_seq_prefix(msg: MessageChain):
    ...
    event = current_event.get()
    mod = cprmc.get()
    if (cache := event.message is msg) and mod:
        return mod

    if msg:
        if isinstance(at := msg[0], At) and at.user_id == event.self_id:
            del (mod := msg.copy())[0]
        if isinstance(text := msg[0], Text) and text.text is not None and cfg.message_prefix:
            if not mod:
                mod = msg.copy()
            mod[0] = text = text.model_copy()

            for prefix in (cfg.message_prefix, convert_text(cfg.message_prefix)):
...
# endregion
def _has_message_prefix(event: Message):
    if cfg.message_prefix:
        return any(
            event.message_str.startswith(prefix)
            for prefix in (cfg.message_prefix, convert_text(cfg.message_prefix), f"[Aha:at,user_id={event.self_id}]")
        )
...
@async_cached(
    LRUCache(cfg.get_config("admin", 32768, _("expr.fields.admin.cache.cfg_comment"), "cache")),
...
async def _is_super(_):
    from utils.api import is_super
...
def _msg2command(event: Message):
    ...
    for m in remove_msg_seq_prefix(event.message):
        if isinstance(m, Text):
            ...
            command.extend(striped for s in splited[0].split(" ") if (striped := s.strip()))
            if splited[1]:
...
def _wrap_conditions(conditions, event_type: EventCategory):
            ...
            if isinstance(expr.right, BoolExpr):
                ...
                for r in expr.right.clauses:
                    ...
                    if expr.left.field.rhs_converter:
                        conds.append(recursion(operand(expr.left, expr.left.field.rhs_converter(r, operand, event_type))))
                    else:
                        conds.append(recursion(operand(expr.left, r)))
                return expr.right.__class__(*conds)

            if type(expr.left) is FieldClause:
                if expr.left.field.binary_semantics:
                    operand = expr.left.field.binary_semantics(expr.__class__, expr.right)
                else:
                    operand = expr.__class__
                if expr.left.field.rhs_converter:
                    return operand(expr.left, expr.left.field.rhs_converter(expr.right, operand, event_type))
    ...
    for expr in processed:
        if expr is None:
            continue
        if isinstance(expr, Equal) and type(expr.left) is FieldClause:
...
async def _get_field_value(field: FieldClause, msg: BaseEvent):
    if field.field.extractor is None and field.field.requires_extractor:
        return True
...
def _adjust_binary_field(left, right) -> tuple[FieldClause | Expr | Any, FieldClause | Expr | Any]:
    # 将 FieldClause 转到左侧
    left_is_field = type(left) is FieldClause
    right_is_field = type(right) is FieldClause
...
class CacheConfig:
    ...
    cache: Any
    key_func: Callable[[str, Any, BaseEvent], Hashable] = lambda operator, right, _: hashkey(operator, right)
...
class PM(metaclass=PatternMatcherMeta):
    ...
    Attributes:
        ...
        prefix: 消息前缀是否为 `cfg.message_prefix` 或@机器人。
    ...
    message: FieldClause[str] = Field(
        lambda event: remove_str_msg_prefix(event.message_str) if isinstance(event, Message) else None,
        operand_types={(LocalizedString, str, re.Pattern): FullMatch},
        rhs_converter=_convert_pattern_rhs,
        cache=CacheConfig(
            MemLRUCache(parse_size(cfg.get_config("message_match", "32MiB", _("expr.fields.msg.cache"), "cache"))),
    ...
    msg: FieldClause[str] = Field(redirect="message")
    message_str: FieldClause[str] = Field(redirect="message")
    msg_str: FieldClause[str] = Field(redirect="message")
    message_chain: FieldClause[MessageChain] = Field(
        ...
        rhs_converter=_convert_chain_rhs,
        operand_types={(type, GenericAlias, UnionType, TypeAdapter, _Final, _UnionGenericAlias): ValidateBy}
    )
    msg_chain: FieldClause[MessageChain] = Field(redirect="message_chain")
    command: FieldClause[list[str | MsgSeg]] = Field(
        _msg2command,
        binary_semantics=_convert_to_singletonof,
        rhs_converter=_convert_command_rhs,
        operand_types={Sequence: Equal}
    )
    # endregion
    # region 类型匹配字段
    type_: FieldClause[EventType] = Field(
        lambda event: event.event_type,
        rhs_converter=_convert_type_rhs,
        operand_types={EventType: Equal},
        priority=39
    )
    request: FieldClause[RequestEventType] = Field(redirect="type_")
    notice: FieldClause[NoticeEventType] = Field(redirect="type_")
    meta: FieldClause[MetaEventType] = Field(redirect="type_")
    ...
    isprivate: FieldClause[bool] = Field(
        ...
        None if cfg.get_config("private", True, _("expr.fields.isprivate.default_cfg"), "aha") else (lambda v: v == False),
        priority=7,
    )
    ...
    # endregion
    # region 功能控制字段
    prefix: FieldClause[bool] = Field(_has_message_prefix, priority=-1)
    admin: FieldClause[bool] = Field(_is_admin, priority=-50)
    super: FieldClause[bool] = Field(_is_super, priority=50)
    validated: FieldClause[bool] = Field(default=lambda v: v == True, requires_extractor=True, priority=-10)
    limit: FieldClause[bool] = Field(
        _check_rate_limit, (lambda v: v == True) if LIMIT else None, overrides={False: True}, priority=-999
    )
    # endregion
...
def check_field_exists(expr: Expr | Any, field: FieldClause | Iterable[FieldClause], binary_expr: type[BinaryExpr] = None):
    """检查表达式中是否存在指定字段

    Args:
        binary_expr (type[BinaryExpr]): 限定字段父级为指定二元表达式类型。
    """
    if isinstance(expr, BoolExpr):
        return any(check_field_exists(c, field, binary_expr) for c in expr.clauses)
    elif isinstance(expr, Not):
        return check_field_exists(expr.clause, field, binary_expr)
    elif isinstance(expr, BinaryExpr):
        return (
            binary_expr
            and (expr.left is field or isinstance(field, Iterable) and any(expr.left is x for x in field))
            and isinstance(expr, binary_expr)
            or check_field_exists(expr.left, field, binary_expr)
            or check_field_exists(expr.right, field, binary_expr)
        )
    elif not binary_expr:
        if expr is field:
            return True
        if isinstance(field, Iterable):
            return any(expr is x for x in field)
    return False
...
def build_cond(conditions: Iterable[Expr | str | re.Pattern], event_type: EventCategory, exp=None, debug=False) -> Expr:
    """构建条件"""
    conditions = _wrap_conditions(conditions, event_type)
    used_field_names = {item for cond in conditions for item in _collect_used_fields(cond)}
    default_clauses = [
        default_value
        ...
        and (default_value := field.default(getattr(PM, name))) is not None
    ]
    ...
    cond.exp = exp if exp is None or exp >= 1000000000 else (time() + exp)
    if debug:
        cond.debug = debug
...
extractor_registrations: dict[FieldClause, dict[str, Callable]] = defaultdict(dict)
...
def register_extractor(field: FieldClause):
    ...
    def decorator(func):
        if (module := caller_module(pattern=MODULE_PATTERN)) in extractor_registrations[field]:
            _logger.warning(_("expr.register_extractor.duplicate") % {"field": field.name, "module": module})
            return func
        extractor_registrations[field][module] = func
...
def redirect_extractors():
    ...
    for field, extractors in extractor_registrations.items():
        field.field.extractor = extractors[cfg.get_config(field.name, Option(extractors.keys()), module="expr_extractors")]
...
if DEBUG:
    ...
    async def evaluate(msg: BaseEvent, expr: Expr | BoolExpr, token: int = None, pool: ExprPool = None) -> bool | None:
        """评估表达式入口"""
        if expr.exp and pool and expr.exp <= time():
            await pool.remove_key(expr)
            return None
        try:
            result = await expr.evaluate(msg)
            if hasattr(expr, "debug"):
    ...
    async def evaluate(msg, expr: Expr | BoolExpr, token=None, pool: ExprPool = None) -> bool | None:
        """评估表达式入口"""
        if expr.exp and pool and expr.exp <= time():
            await pool.remove_key(expr)
            return None
        try:
            if result := await expr.evaluate(msg):
                if pool:
                    if token and token not in pool.token_map:
                        result = None
                    if expr.exp:
