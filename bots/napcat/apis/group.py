...
from utils.string import get_byte_length
...
class GroupAPI(BaseGroupAPI):
    ...
    async def send_group_msg(self, call_id, group_id, msg=None, at=None, reply=None, image=None):
        ...
        return (await self._call_api(call_id, "send_group_msg", {"group_id": group_id, "message": await msg.serialize()}))[
            "message_id"
        ]

    async def send_group_unescape_text(self, call_id, group_id, text):
        return (
            await self._call_api(
                call_id, "send_group_msg", {"group_id": group_id, "message": await Text(text=text).serialize()}
            )
        )["message_id"]
    ...
    async def send_group_music(self, call_id, group_id, platform: MusicPlatform, id):
        return (
            await self._call_api(
                call_id,
                "send_group_msg",
                {"group_id": group_id, "message": await Music(type=platform.value, id=id).serialize()},
            )
        )["message_id"]

    async def send_group_custom_music(self, call_id, group_id, url, image=None, audio=None, title=None, content=None):
        return (
            await self._call_api(
                call_id,
                "send_group_msg",
    ...
    async def send_group_forward_msg(self, call_id, group_id, forward: Forward):
        (d := await self.to_forward_dict(forward))["group_id"] = group_id
        return (await self._call_api(call_id, "send_group_forward_msg", d))["message_id"]

    async def send_group_forward_msg_by_id(self, call_id, group_id, messages):
    ...
    # region 群成员管理
    async def set_group_kick_members(self, call_id, group_id, user_id, reject_add_request=False):
    ...
    async def set_group_kick(self, call_id, group_id, user_id, reject_add_request=False):
        if user_id in {i.user_id for i in (await self.get_group_member_list(self.gen_id(), group_id)) if i.role == "member"}:
            return bool(
                await self._call_api(
                    call_id,
                    "set_group_kick",
    ...
    async def set_group_ban(self, _, group_id, user_id, duration=0):
        ...
        # 解禁
        if member.shut_up_time and member.shut_up_time.timestamp() > time():
            create_task(
    ...
    async def set_group_leave(self, call_id, group_id, is_dismiss=False):
    ...
    async def set_group_add_request(self, call_id, flag, approve, reason=None):
    ...
    async def is_admin(self, call_id, group_id, user_id):
        return any(
            m.user_id == user_id and m.role is not Role.MEMBER for m in (await self.get_group_member_list(call_id, group_id))
        )
    ...
    async def get_essence_msg_list(self, call_id, group_id):
        result = []
        for msg in await self._call_api(call_id, "get_essence_msg_list", {"group_id": group_id}):
            msg["content"] = self.build_message_chain(msg["content"])
    ...
    async def get_file(self, call_id, file_id, file):
        return File.model_validate(await self._call_api(call_id, "get_file", {"file_id": file_id, "file": file}))
    ...
    async def get_group_honor_info(self, call_id, group_id, honor_type: HonorType):
        return GroupChatActivity.model_validate(
    ...
    async def get_group_info_ex(self, call_id, group_id):
    ...
    async def get_group_member_info(self, call_id, group_id, user_id):
        data = await self._call_api(call_id, "get_group_member_info", {"group_id": group_id, "user_id": user_id})
        if level := data.pop("level", None):
            data["activity_level"] = level
        data["shut_up_time"] = data["shut_up_timestamp"]
        if aa := data.pop("qage", None):
            data["account_age"] = aa
        return GroupMemberInfo.model_validate(data)

    async def get_group_member_list(self, call_id, group_id):
        for i in (data := await self._call_api(call_id, "get_group_member_list", {"group_id": group_id})):
            ...
            i["shut_up_time"] = i["shut_up_timestamp"]
            if aa := i.pop("qage", None):
                i["account_age"] = aa
        return GroupMemberList(data)
    ...
    async def get_user_by_groups(self, _, user_id):
        tasks = [create_task(self.get_group_member_list(self.gen_id(), g["group_id"])) for g in (await self.get_group_list())]
    ...
    async def set_group_avatar(self, call_id, group_id, file):
        ...
        raise NotImplementedError("远端暂不支持 set_group_avatar API。")
