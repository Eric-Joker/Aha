...
from utils.string import parse_aha_code
...
class Utils(BaseAPI):
    ...
    async def _msg_event_processor(self, data: dict[str, str | Any]):
        if "raw" in data:
            self.logger.warning("请勿在生产环境中开启 NapCat 网络配置的调试模式，这会产生成倍的无效数据。")
    ...
    async def _array_msg_event_processor(self, data: dict[str, str | Any]):
    ...
    async def _string_msg_event_processor(self, data: dict[str, str | Any]):
        ...
        if isinstance(data["message"], str):
            data["message"] = parse_aha_code(data["message"], CQ_CODE_PATTERN)
        data["message"] = MsgSeq(
            [await self.build_msg_seg(item, data["user_id"], data.get("group_id")) for item in data["message"]]
        )
        return data
    ...
    # region onebot2aha
    def _build_downloadable[T: type[Downloadable] | type[Share] | type[CustomMusic]](
        self: NapCat, item: dict[str, dict], type_: T
    ):
    ...
    async def prepare_upload(i: str | Path, local_srv: bool):
        ...
        if local_srv or not await (path := Path(i)).exists():
    ...
    async def serialize_msg_seg(self, item: MsgSeg):
        ...
        if isinstance(item, CustomMusic):
            if self.is_process_mode:
    ...
    async def serialize_msg(self, data: Iterable | MsgSeg | str | None):
        ...
        raise UnknownMessageTypeError(data.__class__)
    ...
    async def shorter_or_forward(self: NapCat, msg: MsgSeq):
        ...
        if need_split or image_count > 20:
            ...
            return Forward(
                content=MsgSeq(Node(user_id=user_id, nickname=nick, content=MsgSeq(s)) for s in result), bot_id=self.bot_id
            )
