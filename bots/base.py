...
from core.transports import Transport, ClientTransport
from models.api import BaseEvent, MetaEvent, MetaEventType, LifecycleSubType
    ...
    async def _handle_requests(self):
        ...
        while True:
            ...
            try:
                ...
                create_task(self.event_post(EventCategory.RESPONSE, (call_id, make_exc_picklable(e))))
            await sleep(0.001)
    ...
    def parse_retry_config(self, config: dict[RetryConfigKey, int | float | list[dict[RetryConfigKey, Any] | Any]]):
        ...
        for stop_key, stop_value in stop_configs.items():
            ...
            if stop_key in ("stop_any", "stop_all"):
                ...
                result["stop"] = stop_cls(*(stop_value if isinstance(stop_value, list) else [stop_value]))
        ...
        for wait_key, wait_value in wait_configs.items():
            ...
            if wait_key in ("wait_chain", "wait_combine"):
                ...
                result["wait"] = wait_cls(*(wait_value if isinstance(wait_value, list) else [wait_value]))
