...
from ..msg import MsgSeq, MsgSeg
from .utils import HonorType, Role  # , Sex


class BaseEvent(BaseModelConfig):
    bot_id: int = Field(None, exclude=True, repr=False)
    platform: str = Field(None, exclude=True, repr=False)
    api_type: str = Field(None, exclude=True, repr=False)
...
class Message[T: MsgSeg](BaseEvent):
    ...
    message: MsgSeq[T]
    message_str: str = Field(None, init=False, repr=False)
    sender: MessageSender
    # anonymous: AnonymousMessage | None = None
    group_id: Annotated[str, BeforeValidator(str)] | None = None

    @model_validator(mode="after")
    def gen_flat_message(self):
        self.message_str = str(self.message)
        return self
    ...
    async def reply(self, msg: str | MsgSeq | dict = None, at=False, image: str = None) -> str:
        from core.api_service import call_api

        return await call_api("send_msg", user_id=self.user_id, group_id=self.group_id, msg=msg, at=self.user_id if at else None, reply=self.message_id, image=image, bot=self.bot_id)

    async def send(self, msg: str | MsgSeq | dict = None, at=False, image: str = None) -> str:
        from core.api_service import call_api

        return await call_api("send_msg", user_id=self.user_id, group_id=self.group_id, msg=msg, at=self.user_id if at else None, image=image, bot=self.bot_id)
...
class Notice(BaseEvent):
    event_type: NoticeEventType = Field(validation_alias="notice_type")
    sub_type: NoticeSubType | None = None
    group_id: Annotated[str, BeforeValidator(str)] | None = None
...
class HeartbeatStatus(BaseModelConfig):
    app_initialized: bool | None = None
    app_enabled: bool | None = None
    plugins_good: bool | None = None
    app_good: bool | None = None
