...
通过[回调函数装饰器](./订阅与发布事件.md)的参数直接声明的表达式中**未使用** `PM.message`、`PM.message_chain`、`PM.command` 字段，且被装饰函数的 `event` 参数存在指定了 [`Message`](../数据结构/事件对象.md#message) 泛型的类型注解，则会基于此泛型注解生成 `Pmsg_chain.validateby(MessageChain[T])` 表达式并与直接声明的表达式进行逻辑与合并。
...
- **主要流程**：向[注册回调装饰器](./订阅与发布事件.md)传入表达式后，会构造出由表达式实例组成的树，在每次接收到新的事件时通过 `core.expr.evaluate` 函数进行递归评估。
...
当前采用的是 [`AsyncScheduler`](https://apscheduler.readthedocs.io/en/master/api.html#apscheduler.AsyncScheduler)。将于 Python 3.15t 稳定后智能切换至 [`Scheduler`](https://apscheduler.readthedocs.io/en/master/api.html#apscheduler.Scheduler)。
...
实例方法 `to_list` **还原上述过程**并返回列表。字符与对象映射存储在上下文中，因此**仅在同一上下文中才可还原！**且可能存在实例化时原字符串包含已被分配的字符的情况，届时会抛出 `models.exc.CodePointConflictError`，可在此之前通过 `clear_map` 清空映射。
