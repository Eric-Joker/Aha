...
class Scheduler(metaclass=SingletonMeta):
    """所有方法存在两个版本，一个用于长效任务或其调度器，一个用于程序生命周期内的临时任务或其调度器。"""
    ...
    async def get_persist_jobs(self, *, id: UUID = None, task_id: str = None, schedule_id: str = None, metadata: MetadataType = None):
