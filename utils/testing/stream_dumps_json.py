...
from ..typekit import stream_async_json
...
async def run_benchmarks():
    ...
    for name, data in test_cases:
        ...
        # 使用orjson加载验证有效性
        remove_async_generators(data, value := loads(result_v1["result"]))
        try:
...
def find_diff(a, b, path=""):
    if type(a) != type(b):
        return f"类型不同: {type(a).__name__} vs {type(b).__name__} at {path}"
