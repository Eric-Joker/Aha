...
class FastAPIConnection(ServerTransport):
    ...
    def __init__(self, logger=None):
        self.logger = logger or getLogger("AHA (FastAPI)")
        self.server: Server
    ...
    async def open(self, config: dict):
        ...
        app = fastapi_FastAPI()
        SignatureVerificationMiddleware.public_key = await load_public_key(await Path.cwd() / "ed25519.pem")
        ...
        async def startup(self, *args, **kwargs):
            ...
            await FastAPI.send(
                EventCategory.META,
                MetaEvent(
                    bot_id=FastAPI.bot_id,
                    platform=FastAPI.__name__,
...
class FastAPI(BaseBot):
    ...
    @classmethod
    async def send(cls, catrgory, data):
        await cls().event_post(catrgory, data)

    @classmethod
    async def put(cls, key, data, lang=None):
        await cls.send(EventCategory.EXTERNAL, (key, data, lang))
