...
若[配置文件](./统一配置系统.md)中 `aha.execution_mode` 被设置为 thread，则可能会由子线程调用回调；若逻辑不保证线程安全则应声明注册回调装饰器工厂的 `threadable` 参数为 `False`，届时只会在主线程执行回调。
...
传递给[注册回调装饰器工厂](./订阅与发布事件.md#注册回调函数)的多个表达式为并列关系。
