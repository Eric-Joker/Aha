...
[配置文件](#配置与运行)的 `aha` 键下有 `group_list`、`user_list` 键值，表示全局默认的平台用户/群组黑白名单。
...
配置项的注册与读取默认以模块自身为作用域，即使键名与通用模块（即 `aha`）中的键相同。读取配置项时**会先读取模块自己的配置项**，模块不存在该键时回退到 `aha` 模块的键值，最后是[表达式字段提取器](./事件匹配表达式.md#注册-pmvalidated-的提取器)的字段。
...
- `Sequence` 会被视为 `tuple`。
- 这些键名拥有框架赋予的特殊意义：`group_list`、`group_list_mode`、`msg_prefix`、`user_list`、`user_list_mode`
...
获取模块的 `msg_prefix` 配置项的值，如果模块不存在该键则返回 `aha` 模块的 `msg_prefix` 配置项的值。
...
前缀内容由配置中的 `aha.msg_prefix` 设置，可以通过 `msg_prefix` 键为每个模块单独设置，允许值为**任意字符串**或 null。
