...
class BinaryExpr(Expr):
    ...
    async def evaluate(self, msg):
        ...
        return await self._evaluate_wrapper(self.left, self.right, msg)
...
def _wrap_conditions(conditions, msg_type):
    ...
    def convert_expr(expr):
        if isinstance(expr, (And, Or)):
            return expr.__class__(*(convert_expr(c) for c in expr.clauses))
...
def build_cond(conditions: tuple[Expr], msg_type: str) -> Expr:
    """构建条件"""
    conditions = _wrap_conditions(conditions, msg_type)
