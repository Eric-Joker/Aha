...
class Expr[Result]:
    ...
    def apply(self, callable):
        # if callable(obj):
        return Apply(self, callable)
        # raise AhaExprTypeError("apply() requires a callable argument.")
...
class And(BoolExpr):
    async def evaluate(self, event):
        return await async_all(await clause.evaluate(event) for clause in self.clauses)


class Or(BoolExpr):
    async def evaluate(self, event):
        return await async_any(await clause.evaluate(event) for clause in self.clauses)
...
class Not(Expr):
    ...
    async def evaluate(self, event):
        return not await self.clause.evaluate(event)
...
class CacheConfig:
    ...
    Attributes:
        ...
        key_func: 生成缓存键的函数，接收运算符的类、另一操作数和事件实例。
        ignore_cache: 决定是否缓存结果的函数，接收 `评估结果, left, right, 事件实例`，后三项参数以 kwargs 传递。
        skip_cache: 决定是否执行缓存相关逻辑, 第二个参数为另一操作数。
        contextvars: 同时缓存指定的 ContextVar。
    """

    cache: Cache
    key_func: Callable[[type[BinaryExpr], Any, BaseEvent], Hashable] = lambda operator, right, _: hash((operator, right))
    ignore_cache: Callable[[Any, Unpack[CacheIgnoreKwargs]], bool] = None
    skip_cache: Callable[[type[BinaryExpr], Any, BaseEvent], bool] = None
...
class PM(metaclass=PatternMatcherMeta):
    ...
    message: FieldClause[str] = Field(
        ...
        cache=CacheConfig(
            ...
            lambda operator, right, event: hash((operator, right, event.message_str)),
            skip_cache=lambda operator, *_: not issubclass(operator, (FullMatch, Match, Search)),
