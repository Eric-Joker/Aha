
from asyncio import create_task, wait_for
from collections import defaultdict
from collections.abc import Callable, Container, Hashable, Sequence
from contextlib import suppress
from contextvars import ContextVar
from enum import Enum, auto
from logging import getLogger
from random import choice, getrandbits
from re import Match, Pattern
from typing import Any, Literal, get_type_hints, overload

from models.api import BaseEvent, Message, MetaEvent, Notice, Request
from models.core import EventCategory
from models.msg import MsgSeg, MsgSeq
from utils.aio import async_run_func
from utils.misc import caller_module, get_arg_names, get_item_by_index

from .config import Option, cfg
from .expr import (
    PM,
    Expr,
    build_cond,
    check_field_exists,
    cprmc,
    cprms,
    current_module,
    evaluate,
    remove_msg_seq_prefix,
    remove_str_msg_prefix,
)
from .i18n import _, create_translator

__all__ = (
    "on_message",
    "on_notice",
    "on_request",
    "on_meta",
    "on_start",
    "on_shutup",
    "clear_handlers",
    "menu_commands",
    "select_bot",
)


# region 回调容器
class ExprPoolNode[Key: Hashable | Expr]:
    __slots__ = ("key", "value", "token", "attach", "prev", "next")

    def __init__(self, key, value, token, attach):
        self.key: Key = key
        self.value: Callable = value
        self.token: int = token
        self.attach: Any | tuple | Callable = attach
        self.prev: ExprPoolNode[Key] | None = None
        self.next: ExprPoolNode[Key] | None = None


class ExprPool[Key: Hashable | Expr, Attach: Any | tuple | Callable | None](Container[tuple[Key, Callable, Attach]]):
    __slots__ = ("_head", "_tail", "token_map", "_key_nodes")

    def __init__(self):
        self._head: ExprPoolNode[Key] | None = None
        self._tail: ExprPoolNode[Key] | None = None
        self.token_map: dict[int, ExprPoolNode[Key]] = {}
        self._key_nodes: dict[Key, list[ExprPoolNode[Key]]] = defaultdict(list)

    def __len__(self):
        return len(self.token_map)

    def __contains__(self, key: Key):
        return key in self._key_nodes

    def __iter__(self):
        current = self._tail  # 从尾部开始倒序遍历
        while current is not None:
            yield current.key, current.value, current.token, current.attach
            current = current.prev

    def add(self, key: Key, value: Callable, attach: Attach = None):
        node = ExprPoolNode(key, value, token := getrandbits(28), attach)

        # 添加到链表尾部
        if self._tail is None:
            self._head = self._tail = node
        else:
            node.prev = self._tail
            self._tail.next = node
            self._tail = node

        self.token_map[token] = node
        self._key_nodes[key].append(node)

    def remove(self, token):
        if (node := self.token_map.pop(token, None)) is None:
            return

        # 从链中移除节点
        if node.prev:
            node.prev.next = node.next
        else:
            self._head = node.next
        if node.next:
            node.next.prev = node.prev
        else:
            self._tail = node.prev

        # 从容器中移除
        (nodes := self._key_nodes[node.key]).remove(node)
        if not nodes:
            del self._key_nodes[node.key]

    def remove_key(self, key: Key):
        if key not in self._key_nodes:
            return

        for node in self._key_nodes.pop(key):
            # 从链中移除
            if node.prev:
                node.prev.next = node.next
            else:
                self._head = node.next
            if node.next:
                node.next.prev = node.prev
            else:
                self._tail = node.prev

            self.token_map.pop(node.token, None)

    def clear(self):
        """移除所有没有exp属性的key"""
        for key in [key for key in self._key_nodes.keys() if not getattr(key, "exp", None)]:
            self.remove_key(key)


# endregion
current_event: ContextVar[BaseEvent] = ContextVar("event", default=None)
current_match: ContextVar[Match] = ContextVar("match", default=None)  # PM.pattern
current_args: ContextVar[list[str | MsgSeg]] = ContextVar("args", default=None)  # PM.command
current_lang = ContextVar("lang", default=cfg.lang)

start_handlers: list[Callable] = []
clean_handlers: list[Callable] = []
menu_commands: list[tuple[str, Expr, str | None]] = []

if cfg.cache_conv:
    groups: dict[str, dict[str, list]] = defaultdict(dict)  # dict[platform, dict[group_id, list[bot_id]]]
    users: dict[str, dict[str, list]] = defaultdict(dict)  # dict[platform, dict[user_id, list[bot_id]]]

_message_handlers = defaultdict(ExprPool[Expr, Callable[[MsgSeq], MsgSeq]])
_notice_handlers = defaultdict(ExprPool[Expr, None])
_request_handlers = defaultdict(ExprPool[Expr, None])
_meta_handlers = defaultdict(ExprPool[Expr, None])
_external_handlers = defaultdict(ExprPool[Expr, None])

# region Decorators
_message_args = {"event", "match", "args", "localizer"}


@overload
def on_message(
    *conditions: Expr | str | Pattern | Sequence[str | type[MsgSeg]],
    exp: float = None,
    debug=False,
    pre_hook: Callable[[MsgSeq], MsgSeq] = None,
    register_help: dict[str, str | None] = None,
) -> Callable: ...


@overload
def on_message(
    *conditions: Expr | str | Pattern | Sequence[str | type[MsgSeg]],
    callback: Callable,
    exp: float = None,
    debug=False,
    pre_hook: Callable[[MsgSeq], MsgSeq] = None,
    register_help: dict[str, str | None] = None,
) -> None: ...


def on_message(*conditions, exp=None, debug=False, pre_hook=None, callback=None, register_help=None):
    """`Message` 事件回调函数装饰器
    - 被装饰的函数必须是异步协程（`async def`），可以选择性声明以下关键字参数：
    - - `event`: `models.api.Message`对象。
    - - `match`: 若表达式中采用了 `PM.pattern`（包括使用了默认字符串）则传递 `re.Match` 对象，否则为None。
    - - `args`: 若表达式中采用了 `PM.command`（包括使用了默认序列）则传递包含了参数的列表，否则为None。
    - - `localizer`: Callable[[str], str] 本地化方法。

    - 不得跨 Aha 模块包装函数，这会导致本地化逻辑异常，因为本地化键名将采用函数声明位置的模块的。
    - 不得装饰非 Python 实现的函数，因为无法获取参数信息。

    - 本地化方法：
    - - 若表达式的 `PM.pattern` 或 `PM.message` 与 `core.i18n._` 方法的返回值进行 `Equal`(`==`) 运算，
    - - 本地化方法将返回本地化键名在匹配到的消息对应的语言下的翻译结果；
    - - 否则会返回 bot 声明的语言或默认语言下的翻译结果。

    Args:
        exp: 该表达式实例将在几秒钟后/何时销毁，一般用于一次性表达式。若传入的值小于10^9，将会被 `build_cond` 修正为与当前秒级时间戳累加。
        debug: 若 `cfg.expr_debug` 为 `True` ，评估该表达式后会打印整个表达式中所有参与评估的二元表达式的两个操作数和匹配结果。
        pre_hook: 对消息链进行预处理，返回值将作用于表达式评估和回调传参。
        register_help: 将 `key` 注册进 Aha 维护的菜单词条列表，如果 `value` 有值，则将其作为说明。
    """

    def decorator(func: Callable):
        nonlocal conditions
        current_module.set(caller_module())

        # 通过 event 关键字参数的类型注解添加 ValidateBy 条件
        if (ann := get_type_hints(func).get("event")) and (ann := ann.__pydantic_generic_metadata__.get("args")):
            if not any(check_field_exists(c, (PM.msg, PM.command, PM.message_chain)) for c in conditions):
                conditions += (PM.message_chain.validateby(MsgSeq[ann[0]]),)

        conditions = build_cond(conditions, EventCategory.CHAT, exp, debug)

        # 注册菜单
        if register_help:
            help_expr = conditions.modify(
                PM.command == None,
                PM.limit == None,
                PM.message == None,
                PM.message_chain == None,
                PM.prefix == None,
            )
            for k, v in register_help.items():
                menu_commands.append((k, help_expr, v))

        (args := [s for s in get_arg_names(func) if s in _message_args]).sort()
        _message_handlers[frozenset(args)].add(conditions, func, pre_hook)

    return decorator(callback) if callback else decorator


_other_args = {"event", "localizer"}


@overload
def on_ontice[Callback: Callable](
    *conditions: Expr | str, exp: float = None, debug=False
) -> Callable[[Callback], Callback]: ...


@overload
def on_notice(*conditions: Expr | str, callback: Callable, exp: float = None, debug=False) -> None: ...


def on_notice(*conditions, exp=None, debug=False, callback=None):
    """`Notice` 事件回调函数装饰器
    - 被装饰的函数必须是异步协程（`async def`），可以选择性声明以下关键字参数：
    - - `event`: `models.api.Notice`对象。
    - - `localizer`: Callable[[str], str] 本地化方法，返回 bot 声明的语言或默认语言下的翻译结果。

    - 不得跨 Aha 模块包装函数，这会导致本地化逻辑异常，因为本地化键名将采用函数声明位置的模块的。
    - 不得装饰非 Python 实现的函数，因为无法获取参数信息。

    Args:
        exp: 该表达式实例将在几秒钟后/何时销毁，一般用于一次性表达式。若传入的值小于10^9，将会被 `build_cond` 修正为与当前秒级时间戳累加。
        debug: 若 `cfg.expr_debug` 为 `True` ，评估该表达式后会打印整个表达式中所有参与评估的二元表达式的两个操作数和匹配结果。
    """

    def decorator(func: Callable):
        current_module.set(caller_module())
        (args := [s for s in get_arg_names(func) if s in _other_args]).sort()
        _notice_handlers[frozenset(args)].add(build_cond(conditions, EventCategory.NOTICE, exp, debug), func)
        return func

    return decorator(callback) if callback else decorator


@overload
def on_request[Callback: Callable](
    *conditions: Expr | str, exp: float = None, debug=False
) -> Callable[[Callback], Callback]: ...


@overload
def on_request(*conditions: Expr | str, callback: Callable, exp: float = None, debug=False) -> None: ...


def on_request(*conditions, exp=None, debug=False, callback=None):
    """`Request` 事件回调函数装饰器
    - 被装饰的函数必须是异步协程（`async def`），可以选择性声明以下关键字参数：
    - - `event`: `models.api.Request`对象。
    - - `localizer`: Callable[[str], str] 本地化方法，返回 bot 声明的语言或默认语言下的翻译结果。

    - 不得跨 Aha 模块包装函数，这会导致本地化逻辑异常，因为本地化键名将采用函数声明位置的模块的。
    - 不得装饰非 Python 实现的函数，因为无法获取参数信息。

    Args:
        exp: 该表达式实例将在几秒钟后/何时销毁，一般用于一次性表达式。若传入的值小于10^9，将会被 `build_cond` 修正为与当前秒级时间戳累加。
        debug: 若 `cfg.expr_debug` 为 `True` ，评估该表达式后会打印整个表达式中所有参与评估的二元表达式的两个操作数和匹配结果。
    """

    def decorator(func: Callable):
        current_module.set(caller_module())
        (args := [s for s in get_arg_names(func) if s in _other_args]).sort()
        _request_handlers[frozenset(args)].add(build_cond(conditions, EventCategory.REQUEST, exp, debug), func)
        return func

    return decorator(callback) if callback else decorator


@overload
def on_meta[Callback: Callable](*conditions: Expr | str, exp: float = None, debug=False) -> Callable[[Callback], Callback]: ...


@overload
def on_meta(*conditions: Expr | str, callback: Callable, exp: float = None, debug=False) -> None: ...


def on_meta(*conditions, exp=None, debug=False, callback=None):
    """`MetaEvent` 事件回调函数装饰器
    - 被装饰的函数必须是异步协程（`async def`），可以选择性声明以下关键字参数：
    - - `event`: `models.api.MetaEvent`对象。
    - - `localizer`: Callable[[str], str] 本地化方法，返回 bot 声明的语言或默认语言下的翻译结果。

    - 不得跨 Aha 模块包装函数，这会导致本地化逻辑异常，因为本地化键名将采用函数声明位置的模块的。
    - 不得装饰非 Python 实现的函数，因为无法获取参数信息。

    Args:
        exp: 该表达式实例将在几秒钟后/何时销毁，一般用于一次性表达式。若传入的值小于10^9，将会被 `build_cond` 修正为与当前秒级时间戳累加。
        debug: 若 `cfg.expr_debug` 为 `True` ，评估该表达式后会打印整个表达式中所有参与评估的二元表达式的两个操作数和匹配结果。
        pre_hook: 对消息链进行预处理，返回值将作用于表达式评估和回调传参。
    """

    def decorator(func: Callable):
        current_module.set(caller_module())
        (args := [s for s in get_arg_names(func) if s in _other_args]).sort()
        _meta_handlers[frozenset(args)].add(build_cond(conditions, EventCategory.META, exp, debug), func)
        return func

    return decorator(callback) if callback else decorator


_external_args = {"data", "localizer"}


def on_external(key):
    """其他服务请求回调函数装饰器
    - 被装饰的函数必须是异步协程（`async def`），可以选择性声明以下关键字参数：
    - - `data`: 由 API 层上报的数据。
    - - `localizer`: Callable[[str], str] 本地化方法，返回 API 层上报时指定的语言下的翻译结果。

    - 不得跨 Aha 模块包装函数，这会导致本地化逻辑异常，因为本地化键名将采用函数声明位置的模块的。
    - 不得装饰非 Python 实现的函数，因为无法获取参数信息。

    Args:
        key: 区分调用函数的特征id。
    """

    def decorator(func: Callable):
        (args := [s for s in get_arg_names(func) if s in _external_args]).sort()
        _external_handlers[frozenset(args)].add(key, func)
        return func

    return decorator


def on_start(func: Callable = None):
    """用于模块注册 Aha 启动时或模块重载时执行的函数"""
    if func is None:
        return on_start
    start_handlers.append(func)
    return func


def on_shutup(func: Callable = None):
    """用于模块注册 Aha 正常关闭时或模块重载前卸载时执行的函数"""
    if func is None:
        return on_shutup
    clean_handlers.append(func)
    return func


# endregion
# region event processers
def get_api_lang(platform):
    return next((v.get("lang") for p in cfg.bots for k, v in p.items() if k == platform), None)


async def _hook_evaluate(hook, expr, token, v):
    # 清理缓存
    cprms.set(None)
    cprmc.set(None)

    current_event.set(event := current_event.get().model_copy(deep=True))
    event.message = (hook_result := await async_run_func(hook, event.message))
    event.message_str = str(hook_result)
    return await evaluate(event, expr, token, v)


async def process_message(event: Message, ignore_prefix=False):
    current_lang.set(get_api_lang(event.api_type))
    current_event.set(event)

    # 删除过时缓存
    cprmc.set(None)
    cprms.set(None)

    common_striped = None
    for k, v in _message_handlers.items():
        e, m, a, l = "event" in k, "match" in k, "args" in k, "localizer" in k
        for expr, func, token, hook in v:
            if ignore_prefix:
                expr = expr.modify(PM.prefix == False)

            if hook:
                if await (eval_task := create_task(_hook_evaluate(hook, expr, token, v))):
                    kwargs = {}
                    if e:
                        kwargs["event"] = (hooked_event := eval_task.get_context()[current_event]).model_copy(
                            update={
                                "message": eval_task.get_context()[cprmc] or remove_msg_seq_prefix(hooked_event.message),
                                "message_str": eval_task.get_context()[cprms]
                                or remove_str_msg_prefix(hooked_event.message_str),
                            }
                        )
                    if m:
                        kwargs["match"] = eval_task.get_context()[current_match]
                    if a:
                        kwargs["args"] = eval_task.get_context()[current_args]
                    if l:
                        kwargs["localizer"] = create_translator(func.__module__, eval_task.get_context()[current_lang])
                    create_task(func(**kwargs), context=eval_task.get_context())

            elif await evaluate(event, expr, token, v):
                kwargs = {}
                if e:
                    if not common_striped:
                        common_striped = event.model_copy(
                            update={
                                "message": remove_msg_seq_prefix(event.message),
                                "message_str": remove_str_msg_prefix(event.message_str),
                            }
                        )
                    kwargs["event"] = common_striped.model_copy(deep=True)
                if m:
                    kwargs["match"] = current_match.get()
                if a:
                    kwargs["args"] = current_args.get()
                if l:
                    kwargs["localizer"] = create_translator(func.__module__, current_lang.get())
                create_task(func(**kwargs))


async def process_notice(event: Notice):
    current_lang.set(get_api_lang(event.api_type))
    current_event.set(event)

    for k, v in _notice_handlers.items():
        e, l = "event" in k, "localizer" in k
        for expr, func, token, _ in v:
            kwargs = {}
            if e:
                kwargs["event"] = event
            if l:
                kwargs["localizer"] = create_translator(func.__module__, current_lang.get())
            if await evaluate(event, expr, token, v):
                create_task(func(**kwargs))


async def process_request(event: Request):
    current_lang.set(get_api_lang(event.api_type))
    current_event.set(event)

    for k, v in _request_handlers.items():
        e, l = "event" in k, "localizer" in k
        for expr, func, token, _ in v:
            kwargs = {}
            if e:
                kwargs["event"] = event
            if l:
                kwargs["localizer"] = create_translator(func.__module__, current_lang.get())
            if await evaluate(event, expr, token, v):
                create_task(func(**kwargs))


async def process_meta(event: MetaEvent):
    current_lang.set(get_api_lang(event.api_type))
    current_event.set(event)

    for k, v in _meta_handlers.items():
        e, l = "event" in k, "localizer" in k
        for expr, func, token, _ in v:
            kwargs = {}
            if e:
                kwargs["event"] = event
            if l:
                kwargs["localizer"] = create_translator(func.__module__, current_lang.get())
            if await evaluate(event, expr, token, v):
                create_task(func(**kwargs))


async def process_external(key, value, lang=None):
    for k, v in _external_handlers.items():
        d, l = "data" in k, "localizer" in k
        for k, func, _, _ in v:
            kwargs = {}
            if d:
                kwargs["data"] = value
            if l:
                kwargs["localizer"] = create_translator(func.__module__, lang)
            if k == key:
                create_task(func(**kwargs))


async def process_start():
    for t in start_handlers:
        await wait_for(async_run_func(t), 1)
    start_handlers.clear()


async def process_clean():
    logger = getLogger("AHA (clean callback)")
    for t in clean_handlers:
        try:
            await async_run_func(t)
        except KeyboardInterrupt:
            pass
        except Exception as e:
            logger.exception(e)
    clean_handlers.clear()


# endregion
def clear_handlers():
    for h in _message_handlers.values():
        h.clear()
    for h in _notice_handlers.values():
        h.clear()
    for h in _request_handlers.values():
        h.clear()
    for h in _meta_handlers.values():
        h.clear()
    for h in _external_handlers.values():
        h.clear()
    start_handlers.clear()
    clean_handlers.clear()
    menu_commands.clear()


# region router.
class SS(Enum):
    """API 实例选择策略，用于 `select_bot` 函数。

    Attributes:
        DEFAULT: 使用配置文件指定的默认策略
        NTH: 接收到该事件的第n个实例（基于事件来源）
        RANDOM: 接收到该事件的随机实例（相同事件源）
        PLATFORM_NTH: 指定平台的第n个实例
        PLATFORM_RANDOM: 指定平台的随机实例
        NTH_ANY: 在配置文件中的第几个bot实例（跨平台）
        RANDOM_ANY: 随机平台的随机实例
        CONTACT_NTH: 指定平台的包含指定联系人的第n个实例
        CONTACT_RANDOM: 指定平台的包含指定联系人的随机实例
        GROUP_NTH: 指定平台的包含指定群聊的第n个实例
        GROUP_RANDOM: 指定平台的包含指定群聊的随机实例
    """

    DEFAULT = auto()
    PLATFORM_RANDOM = auto()
    NTH_ANY = auto()
    RANDOM_ANY = auto()
    CONTACT_NTH = auto()
    CONTACT_RANDOM = auto()
    GROUP_NTH = auto()
    GROUP_RANDOM = auto()
    NTH = "source"
    RANDOM = "random_same"
    PLATFORM_NTH = "platform_first"


DEFAULT_SELECTOR_STRATEGY = cfg.get_config(
    "api_selector_strategy",
    Option((SS.NTH.value, SS.RANDOM.value, SS.PLATFORM_NTH.value)),
    "调用 API 时选择实例默认策略。选项分别为：第一个收到事件的实例、随机接收到该事件的实例、该事件来源平台（或指定平台）的第一个实例",
    "aha",
)


async def init_conversations():
    from .api_service import bots, call_api

    for i, v in bots.items():
        if v is None:
            continue

        g_task = create_task(call_api("get_group_list", bot=i))
        u_task = create_task(call_api("get_friend_list", bot=i))
        group_list = groups[v.platform]
        user_list = users[v.platform]

        with suppress(NotImplementedError):
            for g in await g_task:
                if (available_bots := group_list.get(g.group_id)) is None:
                    available_bots = group_list[g.group_id] = []
                available_bots.append(i)
        with suppress(NotImplementedError):
            for u in await u_task:
                if (available_bots := user_list.get(u.user_id)) is None:
                    available_bots = user_list[u.user_id] = []
                available_bots.append(i)

    return sum(len(i) for i in groups.values()), sum(len(i) for i in users.values())


@overload
def select_bot(strategy: Literal[SS.NTH], event: BaseEvent = None, *, index: int = 0) -> int: ...


@overload
def select_bot(strategy: Literal[SS.PLATFORM_RANDOM], event: BaseEvent = None, *, platform: str = None) -> int: ...


@overload
def select_bot(
    strategy: Literal[SS.PLATFORM_NTH, SS.DEFAULT], event: BaseEvent = None, *, platform: str = None, index: int = 0
) -> int: ...


@overload
def select_bot(
    strategy: Literal[SS.CONTACT_NTH, SS.GROUP_NTH],
    event: BaseEvent = None,
    *,
    platform: str = None,
    conv_id: str = None,
    index: int = 0,
) -> int: ...


@overload
def select_bot(
    strategy: Literal[SS.CONTACT_RANDOM, SS.GROUP_RANDOM], event: BaseEvent = None, *, platform: str = None, conv_id: str = None
) -> int: ...


@overload
def select_bot(strategy: Literal[SS.NTH_ANY], *, index: int = 0) -> int: ...


@overload
def select_bot(strategy: Literal[SS.RANDOM_ANY]) -> int: ...


def select_bot(strategy=SS.DEFAULT, event=None, *, index=0, platform=None, conv_id=None):
    """依据策略和参数选择 bot 服务实例

    Args:
        strategy: 默认为配置文件中选择的策略。
        event: 未传递所需参数时，从中提取所需值。为 `None` 时尝试从 ContextVar 获取。
    """
    if not event:
        try:
            event = current_event.get()
        except LookupError as e:
            raise RuntimeError(_("no_event_found_in_context")) from e

    match SS(DEFAULT_SELECTOR_STRATEGY) if strategy is SS.DEFAULT else strategy:
        case SS.NTH:
            from .api_service import deduplicators

            return deduplicators[event.platform].cache[hash(event)][index]
        case SS.RANDOM:
            from .api_service import deduplicators

            return choice(deduplicators[event.platform].cache[hash(event)])
        case SS.PLATFORM_NTH:
            from .api_service import platform_bot_map

            return platform_bot_map[platform or event.platform][index]
        case SS.PLATFORM_RANDOM:
            from .api_service import platform_bot_map

            return choice(platform_bot_map[platform or event.platform])
        case SS.NTH_ANY:
            from .api_service import bots

            return get_item_by_index(bots, index)[0]
        case SS.RANDOM_ANY:
            from .api_service import bots

            return choice(tuple(filter(None.__ne__, bots.keys())))
        case SS.CONTACT_NTH:
            if cfg.cache_conv:
                if not conv_id:
                    conv_id = getattr(event, "user_id", None)
                if not platform:
                    platform = event.platform
                try:
                    return users[platform][conv_id][index]
                except KeyError as e:
                    raise KeyError(_("router.select_bot.user404") % {"platform": platform, "conv_id": conv_id}) from e
            raise RuntimeError(_("router.select_bot.403"))
        case SS.CONTACT_RANDOM:
            if cfg.cache_conv:
                if not conv_id:
                    conv_id = getattr(event, "user_id", None)
                if not platform:
                    platform = event.platform
                try:
                    return choice(users[platform][conv_id])
                except KeyError as e:
                    raise KeyError(_("router.select_bot.user404") % {"platform": platform, "conv_id": conv_id}) from e
            raise RuntimeError(_("router.select_bot.403"))
        case SS.GROUP_NTH:
            if cfg.cache_conv:
                if not conv_id:
                    conv_id = getattr(event, "group_id", None)
                if not platform:
                    platform = event.platform
                try:
                    return groups[platform][conv_id][index]
                except KeyError as e:
                    raise KeyError(_("router.select_bot.group404") % {"platform": platform, "conv_id": conv_id}) from e
            raise RuntimeError(_("router.select_bot.403"))
        case SS.GROUP_RANDOM:
            if cfg.cache_conv:
                if not conv_id:
                    conv_id = getattr(event, "group_id", None)
                if not platform:
                    platform = event.platform
                try:
                    return groups[platform][conv_id][index]
                except KeyError as e:
                    raise KeyError(_("router.select_bot.group404") % {"platform": platform, "conv_id": conv_id}) from e
            raise RuntimeError(_("router.select_bot.403"))


# endregion
