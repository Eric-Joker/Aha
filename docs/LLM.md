...
大部分事件相关的对象都声明在 `models.api` 中，所有事件对象都是 `BaseEvent` 的子类，是 [Pydantic Model](https://docs.pydantic.dev/latest/concepts/models/)。
...
> 指定 ID 时请尽可能维护唯一性，避免不同模块之间的<small style="color: gray;">~~默契~~</small>冲突。
