...
if DEBUG := cfg.register("expr_debug", False, _("expr.debug.cfg_comment"), "aha"):
...
# endregion
# region 限速
LIMIT = cfg.register("limit", 3, _("expr.fields.limit.cfg_comment"), "aha")
...
@async_cached(
    LRUCache(cfg.register("admin", 32768, _("expr.fields.admin.cache.cfg_comment"), "cache")),
...
class PM(metaclass=PatternMatcherMeta):
    ...
    message: FieldClause[str] = Field(
        ...
        cache=CacheConfig(
            MemLRUCache(parse_size(cfg.register("message_match", "32MiB", _("expr.fields.msg.cache"), "cache"))),
    ...
    isprivate: FieldClause[bool] = Field(
        ...
        None if cfg.register("private", True, _("expr.fields.isprivate.default_cfg"), "aha") else (lambda v: v == False),
        priority=7,
    )
...
def register_extractor(field: FieldClause):
    ...
    def decorator(func):
        if (module := caller_module(pattern=MODULE_PATTERN)) in (extractors := extractor_registrations[field]):
